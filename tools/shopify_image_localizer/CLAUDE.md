...
- 打包与定时发布规范：`docs/superpowers/specs/2026-05-24-shopify-image-localizer-release-standard-fix.md`（备用旧版：`docs/shopify-image-localizer-exe-release-standard.md`）
...
## EXE 发布门禁
- 打包唯一规范：`docs/superpowers/specs/2026-05-24-shopify-image-localizer-release-standard-fix.md`；不读规范、不传 `--release-standard-read`，打包入口必须失败。
...
## AI自动上品 RPA 上架规范
- **接口集成**：桌面端调用 `api_client` 获取 `/openapi/medias/shopify-image-localizer/ai-listing/tasks` 中 `status = 'completed'` 且未上架的商品，获取详情和图片。
- **登录状态校验**：访问 `https://admin.shopify.com/store/<slug>`。如果发现未登录，自动暂停等待，提醒用户在浏览器中登录，登录成功后再继续。
- **详情页 HTML 编辑**：跳过富文本 Slate 调试，在新建商品页面中定位并点击富文本编辑器右上角的 `<>` (Show HTML) 按钮，直接在暴露出来的 `<textarea>` 中填入 CRO 生成的富文本 HTML，保证排版稳定性。
- **图片上传与变体管理**：
  - carousel 与 detail_image 通过 API 鉴权下载至本地临时目录，用 Playwright `set_input_files` 上传到 `input[type="file"]` 控件。
  - 对于多规格变体（SKU），在页面中点击“This product has multiple options”，循环添加变体项并自动回填变体折算价格与 Product Code。
- **Shopify ID 回传**：保存商品后等待页面自动重定向，从 URL 提取商品数字 ID 并通过 success API 接口写回服务器。
