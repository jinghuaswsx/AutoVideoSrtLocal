...
- **禁止回改**：不要在主线程/同一线程连续跑多个语言。

## 批量 Chrome/CDP 状态污染（2026-05-12 事故，v3.33 → v4.0 修复）

- **现象**：v3.33 批量 8 语言全部失败。第 1 个语言 de 跑到详情图上传时崩溃，第 2+ 语言全在「开始连续替换流程」后立即挂掉。
- **根因**：de 线程在上传过程中崩溃（Chrome/CDP websocket 断开），Chrome 被留在损坏状态。后续语言复用到同一个 Chrome（端口 7777 `_cdp_alive` 仍返回 True），CDP 连接立即失败。批量只在开始时杀一次 Chrome，不感知中间语言的崩溃。
- **修复**：`_run_batch` 中**每个语言迭代开始前**调用 `session.kill_chrome_for_profile()` 杀掉旧 Chrome（原只在批量开始前杀一次）。`ensure_cdp_chrome` 会在下一语言自动重启新 Chrome。cookie/session 在 user data dir 里持久化，不影响登录态。
- **禁止回改**：不要为了节省 Chrome 重启时间把 kill 移回循环外；不要跳过语言间的 Chrome 清理。
