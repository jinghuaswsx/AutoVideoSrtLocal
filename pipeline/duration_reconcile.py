...
def _second_rewrite_attempt_record(
    *,
    current: dict,
    round_number: int,
    before_text: str,
...
def _sentence_progress_payload(position: int, current: dict, phase: str) -> dict:
    return {
        ...
        "second_rewrite_attempted": bool(current.get("second_rewrite_attempted")),
        "second_rewrite_result": current.get("second_rewrite_result", ""),
        "second_rewrite_before_text": current.get("second_rewrite_before_text", ""),
        "second_rewrite_after_text": current.get("second_rewrite_after_text", ""),
        "second_rewrite_selected": current.get("second_rewrite_selected"),
        "second_rewrite_selected_round": current.get("second_rewrite_selected_round"),
        "second_rewrite_tts_duration": current.get("second_rewrite_tts_duration"),
        "second_rewrite_target_duration": current.get("second_rewrite_target_duration"),
        "second_rewrite_duration_ratio": current.get("second_rewrite_duration_ratio"),
        "second_rewrite_delta_pct": current.get("second_rewrite_delta_pct"),
        "second_rewrite_status": current.get("second_rewrite_status", ""),
        "second_rewrite_reason": current.get("second_rewrite_reason", ""),
        "second_rewrite_attempts": list(current.get("second_rewrite_attempts") or []),
...
def _run_second_rewrite_loop(
    *,
    ...
    best_second_round = 0
    second_round_limit = min(max_second_rewrite_rounds, MAX_TTS_REGENERATE_ATTEMPTS)
    base_round_offset = int(current.get("rewrite_rounds", 0) or 0)

    for second_round in range(1, second_round_limit + 1):
        display_round = base_round_offset + second_round
        ...
        _emit_sentence_progress(on_progress, position=position, current=current, phase="second_rewrite_start")
        try:
            ...
            current.setdefault("second_rewrite_attempts", []).append(
                {
                    "round": display_round,
                    ...
                    "tts_duration": float(current.get("tts_duration", 0.0) or 0.0),
                    "duration_ratio": round(float(current.get("duration_ratio", 1.0) or 1.0), 4),
                    "delta_pct": _delta_pct(current["target_duration"], float(current.get("tts_duration", 0.0) or 0.0)),
            ...
            _emit_sentence_progress(on_progress, position=position, current=current, phase="second_rewrite_result")
            continue
        ...
        current["rewrite_rounds"] = display_round
        ...
        current["tts_path"], current_duration = _regenerate_segment(
            ...
            suffix=_candidate_suffix("second_rewrite", second_round),
        )
        ...
        second_attempt = _second_rewrite_attempt_record(
            current=current,
        ...
        second_candidate = _candidate_from_current(current, round_number=display_round)
        if best_second_candidate is None or _candidate_rank(second_candidate) <= _candidate_rank(best_second_candidate):
            best_second_candidate = second_candidate
            best_second_round = display_round
    ...
    if best_second_candidate is None:
        ...
        current["speed"] = 1.0
        current["final_fallback_action"] = "second_rewrite_failed"
        current["second_rewrite_result"] = "rewrite_failed"
        current["best_effort"] = True
        current["best_effort_reason"] = "second_rewrite_all_attempts_failed"
        _emit_sentence_progress(on_progress, position=position, current=current, phase="second_rewrite_result")
        return

    select_second = _candidate_rank(best_second_candidate) <= _candidate_rank(before_candidate)

    _apply_candidate(current, best_second_candidate)
    _mark_selected_attempt(current["attempts"], best_second_round)

    for sra in current.get("second_rewrite_attempts") or []:
        sra["selected"] = bool(select_second and int(sra.get("round", -1)) == best_second_round)

    final_status = current["status"]
    final_text = current.get("text", "")
    target_duration = float(current.get("target_duration", 0.0) or 0.0)
    final_tts_duration = float(current.get("tts_duration", 0.0) or 0.0)
    ratio = duration_ratio(target_duration, final_tts_duration)
    current["second_rewrite_after_text"] = final_text
    current["second_rewrite_tts_duration"] = final_tts_duration
    current["second_rewrite_target_duration"] = target_duration
    current["second_rewrite_duration_ratio"] = ratio
    current["second_rewrite_delta_pct"] = _delta_pct(target_duration, final_tts_duration)
    current["second_rewrite_status"] = final_status
    current["second_rewrite_reason"] = _duration_reason(final_status)
    current["second_rewrite_selected"] = select_second
    current["second_rewrite_selected_round"] = best_second_round if select_second else 0
    ...
    if not select_second:
        ...
        current["second_rewrite_result"] = "not_selected"
        current["best_effort"] = True
        current["best_effort_reason"] = "second_rewrite_candidate_not_selected"
        _emit_sentence_progress(on_progress, position=position, current=current, phase="second_rewrite_result")
        return

    if current["status"] == "ok" and not _semantic_coverage_issue(current):
        current["second_rewrite_result"] = "accepted"
        current["best_effort"] = False
        current.pop("best_effort_reason", None)
        _emit_sentence_progress(on_progress, position=position, current=current, phase="second_rewrite_result")
        return
    ...
    if current["duration_ratio"] < MIN_DURATION_RATIO:
        current["second_rewrite_result"] = "still_short"
        current["status"] = "warning_short"
    else:
        current["second_rewrite_result"] = "still_long"
        current["status"] = _warning_status_for_ratio(current["duration_ratio"])
    current["best_effort"] = True
    current["best_effort_reason"] = "second_rewrite_missed"
    _emit_sentence_progress(on_progress, position=position, current=current, phase="second_rewrite_result")
