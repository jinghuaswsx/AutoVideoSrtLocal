...
def build_product_sku_update_response(
    ...
    *,
    ...
    body = body or {}
    allow_variant_title = False
    if "shopify_variant_title" in body:
        allow_variant_title = can_edit_variant_title_fn(product_id, sku_id)
    try:
        fields = normalize_fields_fn(
            body,
            allow_variant_title=allow_variant_title,
        )
...
def build_product_sku_create_response(
    product_id: int,
    product: dict,
    body: dict | None,
    *,
    ...
    try:
        row = create_product_sku_fn(
            product_id,
            fields,
            edited_by=edited_by,
            shopify_product_id=(product.get("shopifyid") or "").strip() or None,
        )
    except ValueError as exc:
        return ProductSkuUpdateResponse(
            {"error": "invalid_fields", "message": str(exc)},
