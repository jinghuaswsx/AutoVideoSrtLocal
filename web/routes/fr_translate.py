...
def bootstrap_upload():
    return _json_response({"error": "新建法语翻译任务已切换为本地上传，请改用 multipart /api/fr-translate/start"}, 410)

@bp.route("/api/fr-translate/complete", methods=["POST"])
@login_required
def complete_upload():
    return _json_response({"error": "新建法语翻译任务已切换为本地上传，TOS complete 创建任务入口已停用"}, 410)
...
@bp.route("/api/fr-translate/<task_id>/artifact-path")
@login_required
