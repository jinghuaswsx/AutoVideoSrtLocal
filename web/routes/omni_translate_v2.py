...
from web.services import omni_v2_pipeline_runner as omni_pipeline_runner
...
bp = Blueprint("omni_translate_v2", __name__)
...
def _list_enabled_target_langs() -> tuple[str, ...]:
    ...
    try:
        ...
        log.warning("[omni_translate_v2] failed to load enabled languages, falling back", exc_info=True)
...
def _list_filter_langs() -> tuple[str, ...]:
    ...
    try:
        ...
        log.warning("[omni_translate_v2] failed to load enabled languages for filter, falling back", exc_info=True)
        return SUPPORTED_LANGS
    return normalize_enabled_target_langs(enabled)
    ...
    *,
    ...
    try:
        return OmniV2TranslateRunner.pipeline_step_names_for_config(
            stored_or_fixed_plugin_config(task),
            include_analysis=include_analysis,
        )
    except Exception:
        log.warning("[omni_v2] invalid fixed plugin_config, using built-in default steps task=%s", task_id, exc_info=True)
        return OmniV2TranslateRunner.pipeline_step_names_for_config(
            stored_or_fixed_plugin_config({}),
            include_analysis=include_analysis,
        )
...
def _ensure_uploaded_video_thumbnail(task_id: str, video_path: str, task_dir: str) -> str:
    ...
    try:
        ...
        log.warning("[omni_translate_v2] thumbnail generation failed for task %s", task_id, exc_info=True)
        return ""
...
def _fresh_viewable_project_task(task_id: str) -> dict | None:
    try:
        ...
        log.warning("[omni_translate_v2] fresh project state lookup failed task=%s", task_id, exc_info=True)
        return None
...
def _copy_source_video_for_duplicate(
    *,
    ...
    try:
        ...
        log.warning(
            "[omni_translate_v2] TOS backup sync failed after duplicate source copy: %s",
            destination,
            exc_info=True,
        )
...
def _multi_translate_creator_name_expr() -> str:
    try:
        ...
        log.warning("[omni_translate_v2] resolve creator name expr failed; fallback to username", exc_info=True)
        return "u.username"
...
@bp.route("/omni-translate-v2")
@login_required
@permission_required("omni_translate_v2")
def index():
    ...
    return render_template(
        ...
        retention_hours=get_retention_hours("omni_translate_v2"),
    )


@bp.route("/omni-translate-v2/<task_id>")
@login_required
@permission_required("omni_translate_v2")
def detail(task_id: str):
    recover_project_if_needed(task_id, "omni_translate_v2")
    ...
    annotation_state = dict(state)
    annotation_state["plugin_config"] = stored_or_fixed_plugin_config(state)
    return render_template(
        "omni_translate_v2_detail.html",
        project=row,
        state=state,
        ...
        plugin_config_annotation=build_plugin_config_annotation(task_id, annotation_state),
    )


@bp.route("/api/omni-translate-v2/<task_id>/subtitle-preview", methods=["GET"])
@login_required
def subtitle_preview(task_id: str):
    ...
    payload = build_multi_translate_preview_payload(
        ...
        api_base="/api/omni-translate-v2",
    )
...
@bp.route("/api/omni-translate-v2/start", methods=["POST"])
@login_required
def upload_and_start():
    ...
    plugin_config_value = current_fixed_plugin_config()
...
@bp.route("/api/omni-translate-v2/<task_id>/duplicate", methods=["POST"])
@login_required
def duplicate(task_id: str):
    ...
    try:
        ...
        log.exception("[omni_translate_v2] duplicate source copy failed task=%s", task_id)
    ...
    update_kwargs["plugin_config"] = current_fixed_plugin_config()
    for key in (
        "voice_gender",
        "voice_id",
        "subtitle_position",
    ...
    return _json_response({
        "status": "started",
        "task_id": new_task_id,
        "redirect_url": f"/omni-translate-v2/{new_task_id}",
    }, 201)


@bp.route("/api/omni-translate-v2/bootstrap", methods=["POST"])
@login_required
def bootstrap_upload():
    return _json_response({"error": "新建多语种翻译任务已切换为本地上传，请改用 multipart /api/omni-translate-v2/start"}, 410)

@bp.route("/api/omni-translate-v2/complete", methods=["POST"])
@login_required
def complete_upload():
    return _json_response({"error": "新建多语种翻译任务已切换为本地上传，TOS complete 创建任务入口已停用"}, 410)

@bp.route("/api/omni-translate-v2/<task_id>", methods=["GET"])
@login_required
...
@bp.route("/api/omni-translate-v2/<task_id>/llm-debug/<step>", methods=["GET"])
@login_required
...
@bp.route("/api/omni-translate-v2/<task_id>/restart", methods=["POST"])
@login_required
def restart(task_id):
...
@bp.route("/api/omni-translate-v2/<task_id>/start", methods=["POST"])
@login_required
def start(task_id):
    recover_task_if_needed(task_id)
    task = _get_viewable_task(task_id)
...
@bp.route("/api/omni-translate-v2/<task_id>/source-language", methods=["PUT"])
@login_required
...
@bp.route("/api/omni-translate-v2/<task_id>/alignment", methods=["PUT"])
@login_required
...
@bp.route("/api/omni-translate-v2/<task_id>/segments", methods=["PUT"])
@login_required
...
@bp.route("/api/omni-translate-v2/<task_id>/export", methods=["POST"])
@login_required
def export(task_id):
    task = _get_viewable_task(task_id)
    if not task:
...
@bp.route("/api/omni-translate-v2/<task_id>/loudness-profile", methods=["POST"])
@login_required
@permission_required("omni_translate_v2")
...
@bp.route("/api/omni-translate-v2/<task_id>/resume", methods=["POST"])
@login_required
def resume(task_id):
    recover_task_if_needed(task_id)
    task = _get_viewable_task(task_id)
...
@bp.route("/api/omni-translate-v2/<task_id>/download/<file_type>")
@login_required
...
@bp.route("/api/omni-translate-v2/<task_id>/video-ai-review/run", methods=["POST"])
@login_required
...
@bp.route("/api/omni-translate-v2/<task_id>/video-ai-review", methods=["GET"])
@login_required
...
@bp.route("/api/omni-translate-v2/<task_id>", methods=["DELETE"])
@login_required
def delete(task_id):
    """软删除多语种翻译任务。"""
...
@bp.route("/api/omni-translate-v2/<task_id>/visible-to-all", methods=["PUT"])
@login_required
...
@bp.route("/api/omni-translate-v2/<task_id>/artifact/<name>")
@login_required
...
@bp.route("/api/omni-translate-v2/<task_id>/artifact-path")
@login_required
...
@bp.route("/api/omni-translate-v2/<task_id>/round-file/<int:round_index>/attempt/<int:attempt>")
@login_required
...
@bp.route("/api/omni-translate-v2/<task_id>/round-file/<int:round_index>/<kind>")
@login_required
...
@bp.route("/api/omni-translate-v2/<task_id>/analysis/run", methods=["POST"])
@login_required
...
@bp.route("/api/omni-translate-v2/user-default-voice", methods=["PUT"])
@login_required
...
@bp.route("/api/omni-translate-v2/<task_id>/voice", methods=["PUT"])
@login_required
...
@bp.route("/api/omni-translate-v2/<task_id>/voice-library", methods=["GET"])
@login_required
...
@bp.route("/api/omni-translate-v2/<task_id>/rematch", methods=["POST"])
...
@bp.route("/api/omni-translate-v2/<task_id>/voice-ai-ranking", methods=["POST"])
@login_required
@admin_required
...
@bp.route("/api/omni-translate-v2/<task_id>/voice-ai-ranking/force-speed-fallback", methods=["POST"])
@login_required
@admin_required
...
@bp.route("/api/omni-translate-v2/<task_id>/confirm-voice", methods=["POST"])
@login_required
