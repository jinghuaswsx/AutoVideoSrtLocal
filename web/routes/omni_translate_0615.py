...
from web.services import omni_0615_pipeline_runner
...
bp = Blueprint("omni_translate_0615", __name__)
...
def _omni_pipeline_steps_for_task(
    ...
    *,
    ...
    try:
        return OmniTranslate0615Runner.pipeline_step_names_for_config(
            cfg,
...
@bp.route("/omni-translate-0615")
@login_required
@permission_required("omni_translate_0615")
def index():
    ...
    if show_user_filter:
        user_filter_options = translation_route_store.list_project_creators(
            project_type="omni_translate_0615",
    ...
    return render_template(
        ...
        retention_hours=get_retention_hours("omni_translate_0615"),
    )


@bp.route("/omni-translate-0615/<task_id>")
@login_required
@permission_required("omni_translate_0615")
def detail(task_id: str):
    recover_project_if_needed(task_id, "omni_translate_0615")
...
@bp.route("/api/omni-translate-0615/<task_id>/subtitle-preview", methods=["GET"])
@login_required
def subtitle_preview(task_id: str):
    ...
    payload = build_multi_translate_preview_payload(
        ...
        api_base="/api/omni-translate-0615",
    )
...
@bp.route("/api/omni-translate-0615/start", methods=["POST"])
@login_required
def upload_and_start():
    ...
    omni_0615_pipeline_runner.start(task_id, user_id=user_id)
    return _json_response({"task_id": task_id}, 201)


@bp.route("/api/omni-translate-0615/<task_id>/duplicate", methods=["POST"])
@login_required
def duplicate(task_id: str):
    ...
    omni_0615_pipeline_runner.start(new_task_id, user_id=user_id)
    return _json_response({
        "status": "started",
        "task_id": new_task_id,
        "redirect_url": f"/omni-translate-0615/{new_task_id}",
    }, 201)


@bp.route("/api/omni-translate-0615/bootstrap", methods=["POST"])
@login_required
def bootstrap_upload():
    return _json_response({"error": "新建多语种翻译任务已切换为本地上传，请改用 multipart /api/omni-translate-0615/start"}, 410)

@bp.route("/api/omni-translate-0615/complete", methods=["POST"])
@login_required
def complete_upload():
    return _json_response({"error": "新建多语种翻译任务已切换为本地上传，TOS complete 创建任务入口已停用"}, 410)

@bp.route("/api/omni-translate-0615/<task_id>", methods=["GET"])
@login_required
...
@bp.route("/api/omni-translate-0615/<task_id>/llm-debug/<step>", methods=["GET"])
@login_required
...
@bp.route("/api/omni-translate-0615/<task_id>/restart", methods=["POST"])
@login_required
def restart(task_id):
    ...
    updated = restart_task(
        ...
        source_language=raw_source_language,
        user_id=owner_id,
        runner=omni_0615_pipeline_runner,
...
@bp.route("/api/omni-translate-0615/<task_id>/start", methods=["POST"])
@login_required
def start(task_id):
    recover_task_if_needed(task_id)
    task = _get_viewable_task(task_id)
    ...
    omni_0615_pipeline_runner.start(task_id, user_id=owner_id)
    updated_task = store.get(task_id) or task
    return _json_response({"status": "started", "task": updated_task})


@bp.route("/api/omni-translate-0615/<task_id>/source-language", methods=["PUT"])
@login_required
def update_source_language(task_id):
    ...
    omni_0615_pipeline_runner.resume(task_id, start_step, user_id=owner_id)
    return _json_response({
        "status": "started",
...
@bp.route("/api/omni-translate-0615/<task_id>/alignment", methods=["PUT"])
@login_required
def update_alignment(task_id):
    ...
    if task.get("interactive_review"):
        ...
        omni_0615_pipeline_runner.resume(task_id, "translate", user_id=owner_id)
    return _json_response({"status": "ok", "script_segments": script_segments})


@bp.route("/api/omni-translate-0615/<task_id>/segments", methods=["PUT"])
@login_required
def update_segments(task_id):
    ...
    store.set_current_review_step(task_id, "")
    omni_0615_pipeline_runner.resume(task_id, "tts", user_id=owner_id)
    return _json_response({"status": "ok"})


@bp.route("/api/omni-translate-0615/<task_id>/export", methods=["POST"])
@login_required
def export(task_id):
    task = _get_viewable_task(task_id)
    if not task:
        return _json_response({"error": "Task not found"}, 404)
    owner_id = task.get("_user_id") or current_user.id
    omni_0615_pipeline_runner.resume(task_id, "compose", user_id=owner_id)
...
@bp.route("/api/omni-translate-0615/<task_id>/loudness-profile", methods=["POST"])
@login_required
@permission_required("omni_translate_0615")
...
@bp.route("/api/omni-translate-0615/<task_id>/resume", methods=["POST"])
@login_required
def resume(task_id):
    recover_task_if_needed(task_id)
    task = _get_viewable_task(task_id)
...
@bp.route("/api/omni-translate-0615/<task_id>/download/<file_type>")
@login_required
...
@bp.route("/api/omni-translate-0615/<task_id>/video-ai-review/run", methods=["POST"])
@login_required
...
@bp.route("/api/omni-translate-0615/<task_id>/video-ai-review", methods=["GET"])
@login_required
...
@bp.route("/api/omni-translate-0615/<task_id>", methods=["DELETE"])
@login_required
def delete(task_id):
    """软删除多语种翻译任务。"""
...
@bp.route("/api/omni-translate-0615/<task_id>/visible-to-all", methods=["PUT"])
@login_required
...
@bp.route("/api/omni-translate-0615/<task_id>/sentence-tts-loudness-calibration", methods=["PUT"])
@login_required
...
@bp.route("/api/omni-translate-0615/<task_id>/auto-voice-selection", methods=["PUT"])
@login_required
...
@bp.route("/api/omni-translate-0615/<task_id>/artifact/<name>")
@login_required
...
@bp.route("/api/omni-translate-0615/<task_id>/artifact-path")
@login_required
...
@bp.route("/api/omni-translate-0615/<task_id>/round-file/<int:round_index>/attempt/<int:attempt>")
@login_required
...
@bp.route("/api/omni-translate-0615/<task_id>/round-file/<int:round_index>/<kind>")
@login_required
...
@bp.route("/api/omni-translate-0615/<task_id>/analysis/run", methods=["POST"])
@login_required
...
@bp.route("/api/omni-translate-0615/user-default-voice", methods=["PUT"])
@login_required
...
@bp.route("/api/omni-translate-0615/<task_id>/voice", methods=["PUT"])
@login_required
...
@bp.route("/api/omni-translate-0615/<task_id>/voice-library", methods=["GET"])
@login_required
...
@bp.route("/api/omni-translate-0615/<task_id>/rematch", methods=["POST"])
...
@bp.route("/api/omni-translate-0615/<task_id>/voice-ai-ranking", methods=["POST"])
@login_required
@admin_required
...
@bp.route("/api/omni-translate-0615/<task_id>/voice-ai-ranking/force-speed-fallback", methods=["POST"])
@login_required
@admin_required
...
@bp.route("/api/omni-translate-0615/<task_id>/confirm-voice", methods=["POST"])
@login_required
def confirm_voice(task_id: str):
    ...
    try:
        ...
        _runner = OmniTranslate0615Runner(bus=EventBus(), user_id=owner_id)
    ...
    omni_0615_pipeline_runner.resume(task_id, next_step, user_id=owner_id)
