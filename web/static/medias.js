(function() {
  ...
  function validateMaterialFilename(filename, productName, langCode) {
    ...
    let tailStart = -1;
    const assignmentTailMatch = fn.match(LOCALIZED_ASSIGNMENT_TAIL_RE);
    if (assignmentTailMatch) tailStart = assignmentTailMatch[1].length;
    if (tailStart < 0) {
      errs.push('结尾必须是 "-{不含空格的指派字段}-蔡靖华.mp4"');
      return errs;
    }
  ...
  function renderHighlightedFilename(filename, productName, langCode) {
    ...
    if (langCode !== 'en') {
      const lang = (LANGUAGES || []).find(l => l.code === langCode);
      const langZh = (lang && lang.name_zh) || '';
      const strictTailRe = new RegExp(`^-原素材-补充素材[A-Ga-g]?\\(${escapeRegExp(langZh)}\\)-\\S+-蔡靖华\\.mp4$`);
      return [
  ...
  function ensureAiEvaluationRequestModalStyle() {
    ...
    style.textContent = `
      ...
      .ect-ai-inline-result-tag.is-good { background:var(--oc-success-bg, oklch(95% 0.04 165)); color:var(--oc-success-fg, oklch(35% 0.12 155)); }
      .ect-ai-inline-result-tag.is-mid { background:var(--oc-warning-bg, oklch(96% 0.06 90)); color:var(--oc-warning-fg, oklch(43% 0.11 80)); }
      .ect-ai-inline-result-tag.is-bad { background:var(--oc-danger-bg, oklch(96% 0.04 25)); color:var(--oc-danger-fg, oklch(45% 0.16 28)); }
      .ect-ai-inline-result-tag.is-info { background:var(--oc-accent-subtle, oklch(94% 0.04 230)); color:var(--oc-accent, oklch(56% 0.16 230)); }
      .ect-ai-inline-result-tag.is-na { background:var(--oc-bg-muted, oklch(94% 0.010 230)); color:var(--oc-fg-muted, oklch(48% 0.018 230)); }
      .ect-ai-inline-score-card { display:grid; grid-template-columns:auto minmax(140px, 220px); gap:6px 12px; align-items:center; max-width:360px; padding:10px 12px; border:1px solid var(--oc-border, oklch(91% 0.012 230)); border-radius:8px; background:var(--oc-bg, oklch(99% 0.004 230)); }
      .ect-ai-inline-score-label { color:var(--oc-fg-muted, oklch(48% 0.018 230)); font-size:12px; }
      ...
      .ect-ai-inline-score-num span { margin-left:2px; color:var(--oc-fg-subtle, oklch(62% 0.015 230)); font-size:12px; font-weight:500; }
      .ect-ai-inline-score-bar { grid-column:1 / -1; height:5px; border-radius:999px; overflow:hidden; background:var(--oc-bg-muted, oklch(94% 0.010 230)); }
      ...
      .ect-ai-inline-score-card.is-good .ect-ai-inline-score-num, .ect-ai-inline-country-summary.is-good .ect-ai-inline-country-score { color:var(--oc-success-fg, oklch(35% 0.12 155)); }
      .ect-ai-inline-score-card.is-good .ect-ai-inline-score-fill, .ect-ai-inline-country-summary.is-good .ect-ai-inline-score-fill { background:var(--oc-success, oklch(58% 0.13 160)); }
      .ect-ai-inline-score-card.is-mid .ect-ai-inline-score-num, .ect-ai-inline-country-summary.is-mid .ect-ai-inline-country-score { color:var(--oc-warning-fg, oklch(43% 0.11 80)); }
      .ect-ai-inline-score-card.is-mid .ect-ai-inline-score-fill, .ect-ai-inline-country-summary.is-mid .ect-ai-inline-score-fill { background:var(--oc-warning, oklch(75% 0.14 85)); }
      .ect-ai-inline-score-card.is-bad .ect-ai-inline-score-num, .ect-ai-inline-country-summary.is-bad .ect-ai-inline-country-score { color:var(--oc-danger-fg, oklch(45% 0.16 28)); }
      .ect-ai-inline-score-card.is-bad .ect-ai-inline-score-fill, .ect-ai-inline-country-summary.is-bad .ect-ai-inline-score-fill { background:var(--oc-danger, oklch(58% 0.18 28)); }
      .ect-ai-inline-score-card.is-na .ect-ai-inline-score-num, .ect-ai-inline-country-summary.is-na .ect-ai-inline-country-score { color:var(--oc-fg-subtle, oklch(62% 0.015 230)); }
      .ect-ai-inline-score-card.is-na .ect-ai-inline-score-fill, .ect-ai-inline-country-summary.is-na .ect-ai-inline-score-fill { background:var(--oc-border-strong, oklch(84% 0.015 230)); }
      .ect-ai-inline-summary-list { display:grid; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); gap:10px; }
      .ect-ai-inline-country-summary { min-width:0; border:1px solid var(--oc-border, oklch(91% 0.012 230)); border-radius:8px; padding:10px; background:var(--oc-bg, oklch(99% 0.004 230)); display:grid; gap:7px; }
      .ect-ai-inline-country-top { display:flex; align-items:baseline; justify-content:space-between; gap:8px; min-width:0; }
      .ect-ai-inline-country-name { color:var(--oc-fg, oklch(22% 0.020 235)); font-size:13px; font-weight:700; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
      ...
      .ect-ai-inline-country-score span { color:var(--oc-fg-subtle, oklch(62% 0.015 230)); font-size:11px; font-weight:500; }
      .ect-ai-inline-country-text { color:var(--oc-fg, oklch(22% 0.020 235)); font-size:12px; line-height:1.55; overflow-wrap:anywhere; }
  ...
  function renderGrid(items) {
    ...
    grid.querySelectorAll('[data-product-unsuitable-push]').forEach(b =>
      b.addEventListener('click', (e) => {
        e.stopPropagation();
  ...
  function manualSkuDraftRowHtml() {
    return `<tr data-new-sku-row="1" class="is-manual is-draft">
      <td>${skuInput('shopify_variant_title', '', '')}</td>
      <td class="mono">${skuInput('shopify_sku', '', 'mono')}</td>
      <td class="mono">${skuNumberInput('shopify_price', '', 'mono')}</td>
      <td class="mono">${skuNumberInput('shopify_inventory_quantity', '', 'mono int')}</td>
      <td class="mono">${skuInput('dianxiaomi_sku', '', 'mono')}</td>
      <td class="mono">${skuInput('dianxiaomi_product_sku', '', 'mono')}</td>
  ...
  function addManualSkuDraftRow() {
    if (!_skuDetailProductId) return;
    const tbody = document.getElementById('skuDetailRows');
    const empty = document.getElementById('skuDetailEmpty');
    if (!tbody) return;
    const existing = tbody.querySelector('[data-new-sku-row]');
    if (existing) {
      const firstInput = existing.querySelector('[data-sku-edit-field]');
      if (firstInput) firstInput.focus();
      return;
    }
    if (empty) empty.hidden = true;
    tbody.insertAdjacentHTML('beforeend', manualSkuDraftRowHtml());
    const draft = tbody.querySelector('[data-new-sku-row]');
    const firstInput = draft && draft.querySelector('[data-sku-edit-field="shopify_variant_title"]');
    if (firstInput) firstInput.focus();
  }
  ...
  async function saveSkuDetailRow(btn) {
    ...
    if (!tr || !skuId) return;
    const body = collectSkuRowBody(tr);
  ...
  async function createSkuDetailRow(btn) {
    if (!_skuDetailProductId || !btn) return;
    const tr = btn.closest('tr[data-new-sku-row]');
    if (!tr) return;
    const body = collectSkuRowBody(tr);
    const original = btn.innerHTML;
    btn.disabled = true;
    btn.innerHTML = `${icon('check', 12)}<span>保存中</span>`;
    try {
      const data = await fetchJSON(`/medias/api/products/${_skuDetailProductId}/skus`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
      });
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const addManual = document.getElementById('skuDetailAddManual');
    const mask = document.getElementById('skuDetailMask');
    if (close) close.addEventListener('click', closeSkuDetail);
    if (cancel) cancel.addEventListener('click', closeSkuDetail);
    if (addManual) addManual.addEventListener('click', addManualSkuDraftRow);
    ...
    if (rows) rows.addEventListener('click', (e) => {
      ...
      const createBtn = e.target.closest && e.target.closest('[data-create-sku-row]');
      if (createBtn) createSkuDetailRow(createBtn);
      const cancelBtn = e.target.closest && e.target.closest('[data-cancel-new-sku-row]');
      if (cancelBtn) {
        const tr = cancelBtn.closest('[data-new-sku-row]');
        if (tr) tr.remove();
        const tbody = document.getElementById('skuDetailRows');
        const empty = document.getElementById('skuDetailEmpty');
        if (empty && tbody && !tbody.children.length) empty.hidden = false;
      }
    });
  ...
  function productUnsuitableType(data, type) {
    const types = Array.isArray(data?.types) ? data.types : [];
    return types.find((item) => item && item.type === type) || null;
  }

  function productUnsuitablePushPreviewJson(data) {
    return JSON.stringify(data?.payload || {}, null, 2);
  }

  function productUnsuitableTypeJson(data, type) {
    const item = productUnsuitableType(data, type);
    return JSON.stringify(item?.payload || {}, null, 2);
  }

  function renderProductUnsuitablePushPanel(data, type) {
    const item = productUnsuitableType(data, type);
    if (!item) {
      return '<div class="oc-pl-empty danger">缺少对应 type 报文</div>';
    }
    const payload = item.payload || {};
    const count = type === 'copy'
      ? ((payload.texts || []).length || 0)
      : ((payload.product_links || []).length || 0);
    const preview = type === 'copy'
      ? ((payload.texts || [])[0] || {})
      : { handle: payload.handle || '', link: (payload.product_links || [])[0] || '' };
    const rows = [
      ['type', item.type || '—'],
      ['接口', item.target_url || '—'],
      ['数量', String(count)],
    ...
    if (type === 'copy') {
      rows.push(['语言', preview.lang || '英语']);
      rows.push(['标题', preview.title || '—']);
      rows.push(['文案', preview.message || '—']);
      rows.push(['描述', preview.description || '—']);
    } else {
      rows.push(['handle', preview.handle || '—']);
      rows.push(['英语链接', preview.link || '—']);
    }
    return `<div class="oc-pl-info-block">
      <dl class="oc-pl-info-list oc-unsuitable-kv">
        ${rows.map(([label, value]) => `<dt>${escapeHtml(label)}</dt><dd>${escapeHtml(value || '—')}</dd>`).join('')}
      </dl>
    </div>`;
  }

  function resetProductUnsuitablePushResponse() {
    const resp = $('productUnsuitablePushResponse');
    const respPre = $('productUnsuitablePushResponseBody');
    const respTitle = $('productUnsuitablePushResponseTitle');
    if (resp) {
      resp.hidden = true;
      resp.classList.remove('success', 'danger');
    }
  ...
  function productUnsuitablePushRenderResponse(data, forceSuccess) {
    const resp = $('productUnsuitablePushResponse');
    const respPre = $('productUnsuitablePushResponseBody');
    const respTitle = $('productUnsuitablePushResponseTitle');
    if (!resp || !respPre) return false;
  ...
  async function openProductUnsuitablePushModal(product) {
    if (!product || !product.id) return;
    const mask = $('productUnsuitablePushModalMask');
    const title = $('productUnsuitablePushTitle');
    const meta = $('productUnsuitablePushMeta');
    const info = $('productUnsuitablePushInfo');
    const copyInfo = $('productUnsuitableCopyInfo');
    const copyJson = $('productUnsuitableCopyJson');
    const linksInfo = $('productUnsuitableLinksInfo');
    const linksJson = $('productUnsuitableLinksJson');
    const copySubmit = $('productUnsuitableCopySubmit');
    const linksSubmit = $('productUnsuitableLinksSubmit');
    const submit = $('productUnsuitablePushSubmit');
    if (!mask || !copyInfo || !copyJson || !linksInfo || !linksJson || !submit) return;

    mask.hidden = false;
    mask.dataset.pid = String(product.id);
    resetProductUnsuitablePushResponse();
    if (title) title.textContent = '推送不合适产品';
    if (meta) meta.textContent = `${product.name || ''} · ${product.product_code || ''}`;
    if (info) info.innerHTML = '<div class="oc-pl-empty">加载产品信息中…</div>';
    copyInfo.innerHTML = '<div class="oc-pl-empty">加载推送文案中…</div>';
    linksInfo.innerHTML = '<div class="oc-pl-empty">加载推送链接中…</div>';
    copyJson.classList.remove('danger');
    linksJson.classList.remove('danger');
    copyJson.textContent = '加载中…';
    linksJson.textContent = '加载中…';
    if (copySubmit) {
      copySubmit.disabled = true;
      copySubmit.textContent = '推送文案';
    }
    if (linksSubmit) {
      linksSubmit.disabled = true;
      linksSubmit.textContent = '推送链接';
    }
    submit.disabled = true;
    submit.textContent = '一键推送';
    ...
    try {
      ...
      copyInfo.innerHTML = renderProductUnsuitablePushPanel(data, 'copy');
      linksInfo.innerHTML = renderProductUnsuitablePushPanel(data, 'links');
      copyJson.textContent = productUnsuitableTypeJson(data, 'copy');
      linksJson.textContent = productUnsuitableTypeJson(data, 'links');
      const canPush = Array.isArray(data.types) && data.types.length === 2;
      if (copySubmit) copySubmit.disabled = !productUnsuitableType(data, 'copy');
      if (linksSubmit) linksSubmit.disabled = !productUnsuitableType(data, 'links');
      submit.disabled = !canPush;
    } catch (err) {
      const message = err.message || '加载失败';
      if (info) info.innerHTML = `<div class="oc-pl-empty danger">${escapeHtml(message)}</div>`;
      copyInfo.innerHTML = `<div class="oc-pl-empty danger">${escapeHtml(message)}</div>`;
      linksInfo.innerHTML = `<div class="oc-pl-empty danger">${escapeHtml(message)}</div>`;
      copyJson.classList.add('danger');
      linksJson.classList.add('danger');
      copyJson.textContent = JSON.stringify({ error: message }, null, 2);
      linksJson.textContent = JSON.stringify({ error: message }, null, 2);
      if (copySubmit) copySubmit.disabled = true;
      if (linksSubmit) linksSubmit.disabled = true;
      submit.disabled = true;
    }
  }

  function closeProductUnsuitablePushModal() {
    const mask = $('productUnsuitablePushModalMask');
    if (!mask) return;
    mask.hidden = true;
    mask.dataset.pid = '';
    mask._productUnsuitablePreview = null;
    resetProductUnsuitablePushResponse();
  }

  async function submitProductUnsuitablePushType(pushType) {
    const mask = $('productUnsuitablePushModalMask');
    const submit = pushType === 'copy'
      ? $('productUnsuitableCopySubmit')
      : (pushType === 'links' ? $('productUnsuitableLinksSubmit') : $('productUnsuitablePushSubmit'));
    const resp = $('productUnsuitablePushResponse');
    const respPre = $('productUnsuitablePushResponseBody');
    const respTitle = $('productUnsuitablePushResponseTitle');
    const pid = mask && mask.dataset.pid;
    if (!mask || !pid || !submit) return;
    submit.disabled = true;
    const defaultLabel = pushType === 'copy' ? '推送文案' : (pushType === 'links' ? '推送链接' : '一键推送');
    submit.textContent = '推送中…';
    if (resp) {
      resp.hidden = false;
      resp.classList.remove('success', 'danger');
    ...
    try {
      const data = await fetchJSON(`/medias/api/products/${pid}/product-unsuitable-push`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: pushType ? JSON.stringify({ type: pushType }) : undefined,
      });
      const success = productUnsuitablePushRenderResponse(data);
      if (success) {
        submit.textContent = '已推送';
      } else {
        submit.disabled = false;
      ...
      productUnsuitablePushRenderResponse({
        error: err.error || 'request_failed',
        message: err.message || '推送失败',
        upstream_status: err.upstream_status,
        upstream_code: err.upstream_code,
  ...
  async function submitProductUnsuitablePush() {
    await submitProductUnsuitablePushType('');
  }
  ...
  window.submitProductUnsuitablePushType = submitProductUnsuitablePushType;
  window.submitProductUnsuitablePush = submitProductUnsuitablePush;
  window.closeProductUnsuitablePushModal = closeProductUnsuitablePushModal;
  ...
  function renderPager(total, page, pageSize) {
    ...
    buttons.push(`
      <label class="oc-vm-page-jump">
        <span>去</span>
        <input type="number" min="1" max="${pages}" value="${currentPage}" inputmode="numeric" pattern="[0-9]*" data-page-jump aria-label="跳转到指定页">
        <span>页</span>
      </label>
    `);
  ...
  async function edHandleSimpleLinkProbe() {
    const lang = edState.activeLang;
    const url = edCurrentLinkUrl(lang);
    if (!url || !/^https?:\/\//i.test(url)) {
      alert('请先填写有效的商品链接');
    ...
    const box = $('edLinkCheckSummary');
    const viewBtn = $('edLinkCheckViewBtn');
    const origText = actionBtn ? actionBtn.textContent : '检查链接';
    ...
    if (box) {
      box.hidden = false;
      box.innerHTML = `<span class="oc-link-check-meta">正在检测中...</span>`;
    }
    if (viewBtn) {
      viewBtn.hidden = true; // hide view report button for simple probe
    }
    ...
    try {
      ...
      const res = await fetchJSON('/medias/api/products/probe-link', {
        method: 'POST',
        headers: headers,
      ...
      if (box) {
        const parts = [];
        if (res.ok) {
          parts.push(edLinkCheckBadge('链接正常', 'success'));
          if (res.http_status) {
            parts.push(edLinkCheckBadge(`HTTP ${res.http_status}`, 'success'));
          }
        } else {
          parts.push(edLinkCheckBadge(res.error || '链接异常', 'danger'));
          if (res.http_status) {
            parts.push(edLinkCheckBadge(`HTTP ${res.http_status}`, 'danger'));
          }
        }
        if (res.elapsed_ms != null) {
          parts.push(`<span class="oc-link-check-meta">耗时 ${res.elapsed_ms}ms</span>`);
        }
        box.innerHTML = parts.join('');
      }
    } catch (e) {
      if (box) {
        box.innerHTML = edLinkCheckBadge('检测失败: ' + (e.message || e), 'danger');
      }
    } finally {
      if (actionBtn) {
        actionBtn.disabled = false;
    ...
    g.innerHTML = (items || []).map(it => {
      ...
      <div class="oc-vitem" data-item="${it.id}" data-lang="${escapeHtml(it.lang || edState.activeLang || 'en')}" data-filename="${escapeHtml(rawName)}">
        ...
        <div class="vactions vactions-vr">
          <button class="oc-btn primary sm" data-act="vr-run" title="多模态 AI 视频分析（Vertex Gemini）—— 首次自动跑、再次只看结果，想重跑去弹窗里点「重新评估」">
...
(function () {
  ...
  document.addEventListener('click', async (event) => {
    ...
    if (event.target.closest('#productUnsuitableCopySubmit')) {
      event.preventDefault();
      await window.submitProductUnsuitablePushType('copy');
      return;
    }

    if (event.target.closest('#productUnsuitableLinksSubmit')) {
      event.preventDefault();
      await window.submitProductUnsuitablePushType('links');
      return;
    }

    if (event.target.closest('#productUnsuitablePushSubmit')) {
      event.preventDefault();
      await window.submitProductUnsuitablePush();
      return;
    }
    ...
    if (
      event.target === $('productUnsuitablePushModalMask')
      || event.target.closest('#productUnsuitablePushClose')
    ) {
      window.closeProductUnsuitablePushModal();
      return;
    }
...
(function() {
  ...
  document.getElementById('supplementTitle').textContent = `补素材 — ${productName || ''}`;
  document.getElementById('supplementProductCode').textContent = productCode || '';

  const container = document.getElementById('supplementCardsContainer');
  const summary = document.getElementById('supplementSummaryInfo');
  container.innerHTML = '<div class="supplement-loading"><div class="oc-skel" style="height:200px;"></div><div class="oc-skel" style="height:200px;"></div><div class="oc-skel" style="height:200px;"></div></div>';
  summary.innerHTML = '<span class="muted">加载中…</span>';

  fetchJSON(`/medias/api/product/${productId}/supplement`)
    .then(data => {
      renderSupplementSummary(data, summary);
      renderSupplementCards(data, container);
    })
    .catch(err => {
      container.innerHTML = `<div class="supplement-error">加载失败：${escapeHtml(err.message || '请稍后重试')}</div>`;
      summary.innerHTML = '';
    });
}
...
function renderSupplementSummary(data, el) {
  const s = data.summary || {};
  const langCov = data.lang_coverage || {};
  const langList = Object.keys(langCov).filter(k => k !== 'en').map(k => {
    const lc = langCov[k];
    const status = lc.delivery_status || 'never';
    const statusLabel = status === 'active' ? '投放中' : status === 'stopped' ? '终止' : '未投放';
    return `<span class="supplement-lang-summary supplement-lang-${status}">${k.toUpperCase()} ${statusLabel}</span>`;
  }).join(' ');
  el.innerHTML = `
    <span>明空素材: <b>${s.total_mk_videos || 0}</b></span>
    <span class="supplement-sep">·</span>
    <span>已入库: <b style="color:#22c55e;">${s.in_library || 0}</b></span>
    <span class="supplement-sep">·</span>
    <span>未入库: <b style="color:#94a3b8;">${s.not_in_library || 0}</b></span>
    ${langList ? `<span class="supplement-sep">·</span><span>语种覆盖: ${langList}</span>` : ''}
  `;
}

function renderSupplementCards(data, container) {
  const cards = data.cards || [];
  if (!cards.length) {
    container.innerHTML = '<div class="supplement-empty">明空选品中心未找到该产品的视频素材</div>';
    return;
  }
  container.innerHTML = cards.map(card => renderSupplementCard(card, data.product)).join('');
}

function renderSupplementCard(card, product) {
  const inLib = card.in_library;
  const mk = card.mk_video || {};
  const tagClass = inLib ? 'supplement-tag-in' : 'supplement-tag-out';
  const tagText = inLib ? '已入库' : '未入库';

  // Cover image: prefer local cache, fallback to MK proxy
  const localCoverKey = mk.local_cover_object_key || '';
  const coverPath = mk.image_path || '';
  let coverUrl = '';
  if (localCoverKey) {
    coverUrl = `/medias/object?object_key=${encodeURIComponent(localCoverKey)}`;
  } else if (coverPath) {
    coverUrl = `/xuanpin/api/mk-media?path=${encodeURIComponent(coverPath)}`;
  }
  const coverHtml = coverUrl
    ? `<img src="${escapeHtml(coverUrl)}" alt="" loading="lazy" class="supplement-card-cover">`
    : '<div class="supplement-card-cover-ph"><svg width="32" height="32"><use href="#ic-film"/></svg></div>';

  // MK info
  const spends = mk.spends != null ? `$${Number(mk.spends).toFixed(2)}` : '—';
  const adsCount = mk.ads_count || 0;
  const author = mk.author || '';
  const uploadTime = mk.upload_time || '';
  const videoName = mk.name || '';

  // Translated versions for in-library cards
  let versionsHtml = '';
  if (inLib && card.translated_versions && card.translated_versions.length) {
    versionsHtml = '<div class="supplement-versions">' +
      card.translated_versions.map(v => {
        const status = v.delivery_status || 'never';
        const statusLabel = status === 'active' ? '投放中' : status === 'stopped' ? '终止' : '未投放';
        const spend = v.ad_spend != null ? `$${Number(v.ad_spend).toFixed(0)}` : '';
        const roas = v.roas != null ? `R${Number(v.roas).toFixed(1)}` : '';
        const detail = [spend, roas].filter(Boolean).join(' ');
        return `<span class="supplement-version-pill supplement-version-${status}" data-item-id="${v.media_item_id || ''}" data-lang="${escapeHtml(v.lang || '')}" data-product-id="${product ? product.id : ''}" title="点击展开详细数据">
          ${escapeHtml((v.lang || '').toUpperCase())} ${statusLabel}${detail ? ' ' + detail : ''}
        </span>`;
      }).join('') +
      '</div>';
  }
  ...
  if (inLib) {
    actionsHtml = `
      <div class="supplement-card-actions">
        <button class="oc-btn sm primary supplement-action-translate"
                data-product-id="${product ? product.id : ''}"
                data-item-id="${card.media_item_id || ''}"
                title="创建小语种翻译任务">创建翻译任务</button>
        <a class="oc-btn sm ghost" href="/medias/video?keyword=${encodeURIComponent(videoName)}" target="_blank" rel="noopener noreferrer" title="查看素材详情">查看详情</a>
      </div>`;
  } else {
    // For not-in-library cards, provide import button
    const mkProductId = card.mk_product_id || '';
    const mkProductName = card.mk_product_name || '';
    const videoPath = mk.path || '';
    const productCode = product ? (product.mk_search_handle || product.product_code || '') : '';
    actionsHtml = `
      <div class="supplement-card-actions">
        <button class="oc-btn sm primary supplement-action-import"
                data-mk-product-id="${escapeHtml(String(mkProductId))}"
                data-mk-product-name="${escapeHtml(mkProductName)}"
                data-video-path="${escapeHtml(videoPath)}"
                data-video-name="${escapeHtml(videoName)}"
                data-cover-path="${escapeHtml(coverPath)}"
                data-product-code="${escapeHtml(productCode)}"
                data-product-link="${escapeHtml((card.mk_video || {}).product_link || '')}"
                data-main-image="${escapeHtml((card.mk_video || {}).main_image || '')}"
                title="加入素材库">加入素材库</button>
        <button class="oc-btn sm ghost supplement-action-mk-detail"
                data-mk-search="${encodeURIComponent(productCode)}"
                title="在明空选品中心查看">明空查看</button>
      </div>`;
  }

  return `
    <div class="supplement-card ${inLib ? 'supplement-card-in' : 'supplement-card-out'}" data-card-id="${escapeHtml(card.card_id || '')}">
      <div class="supplement-card-tag ${tagClass}">${tagText}</div>
      <div class="supplement-card-body">
        <div class="supplement-card-media">
          ${coverHtml}
        </div>
        <div class="supplement-card-info">
          <div class="supplement-card-name" title="${escapeHtml(videoName)}">${escapeHtml(videoName)}</div>
          <div class="supplement-card-meta">
            <span>广告数: ${adsCount}</span>
            <span>消耗: ${spends}</span>
            ${author ? `<span>上传: ${escapeHtml(author)}</span>` : ''}
            ${uploadTime ? `<span>时间: ${escapeHtml(uploadTime)}</span>` : ''}
          </div>
          ${versionsHtml}
          <div id="supplement-detail-${escapeHtml(card.card_id || '')}" class="supplement-detail-panel" hidden></div>
          ${actionsHtml}
        </div>
      </div>
    </div>`;
...
// Handle version pill click to expand ad detail
function toggleSupplementVersionDetail(pill) {
  const itemId = pill.dataset.itemId;
  const lang = pill.dataset.lang;
  const productId = pill.dataset.productId;
  if (!productId || !lang) return;

  const card = pill.closest('.supplement-card');
  if (!card) return;
  const detailPanel = card.querySelector('.supplement-detail-panel');
  if (!detailPanel) return;

  if (!detailPanel.hidden && detailPanel.dataset.activeLang === lang) {
    detailPanel.hidden = true;
    pill.classList.remove('active');
    return;
  }

  // Deactivate all pills in this card
  card.querySelectorAll('.supplement-version-pill').forEach(p => p.classList.remove('active'));
  pill.classList.add('active');

  detailPanel.hidden = false;
  detailPanel.dataset.activeLang = lang;
  detailPanel.innerHTML = '<div class="supplement-detail-loading">加载中…</div>';

  fetchJSON(`/medias/api/video-material/${productId}/ad-detail?lang=${encodeURIComponent(lang)}`)
    .then(data => {
      detailPanel.innerHTML = renderSupplementAdDetail(data);
    })
    .catch(err => {
      detailPanel.innerHTML = `<div class="supplement-detail-error">加载失败: ${escapeHtml(err.message || '')}</div>`;
    });
}

function renderSupplementAdDetail(data) {
  const spend = v => v != null ? `$${Number(v).toFixed(2)}` : '—';
  const roas = v => v != null ? Number(v).toFixed(2) : '—';
  const count = v => v != null ? String(v) : '—';
  return `
    <table class="supplement-detail-table">
      <thead><tr><th>指标</th><th>值</th></tr></thead>
      <tbody>
        <tr><td>广告消耗</td><td>${spend(data.ad_spend_usd)}</td></tr>
        <tr><td>购买金额</td><td>${spend(data.purchase_value_usd)}</td></tr>
        <tr><td>ROAS</td><td>${roas(data.ad_roas)}</td></tr>
        <tr><td>最近7天消耗</td><td>${spend(data.active_7d_ad_spend_usd)}</td></tr>
        <tr><td>已推送视频数</td><td>${count(data.pushed_video_count)}</td></tr>
        <tr><td>投放状态</td><td>${escapeHtml(data.delivery_status === 'active' ? '投放中' : data.delivery_status === 'stopped' ? '终止' : '未投放')}</td></tr>
      </tbody>
    </table>`;
}

// Handle import button click
async function supplementImportMaterial(btn) {
  ...
  try {
    const rsp = await fetch('/mk-import/video', {
      ...
      body: JSON.stringify({
        mk_video_metadata: {
          mk_id: parseInt(btn.dataset.mkProductId) || null,
          product_name: btn.dataset.mkProductName || '',
          product_code: btn.dataset.productCode || '',
          product_link: btn.dataset.productLink || null,
          video_path: btn.dataset.videoPath || null,
          cover_path: btn.dataset.coverPath || null,
          filename: btn.dataset.videoName || '',
          main_image: btn.dataset.mainImage || null,
        }
      }),
    });
    if (!rsp.ok) {
      const err = await rsp.json().catch(() => ({error: rsp.statusText}));
      throw new Error(err.detail || err.error || rsp.statusText);
    }
    btn.textContent = '✓ 已入库';
    ...
    if (card) {
      card.querySelector('.supplement-card-tag').textContent = '已入库';
      card.querySelector('.supplement-card-tag').className = 'supplement-card-tag supplement-tag-in';
      card.className = card.className.replace('supplement-card-out', 'supplement-card-in');
    }
  } catch (e) {
    btn.disabled = false;
    btn.textContent = original;
    window.alert('入库失败：' + e.message);
  }
}

// Handle translate button click - redirect to MK selection page
function supplementCreateTranslation(btn) {
  const productId = btn.dataset.productId;
  const itemId = btn.dataset.itemId;
  if (!productId) return;
  // Navigate to MK selection page where existing translation task flow is available
  const code = document.getElementById('supplementProductCode');
  const productCode = code ? code.textContent.replace(/-rjc$/i, '') : '';
  if (productCode) {
    window.open(`/xuanpin/mk?q=${encodeURIComponent(productCode)}&range=all`, '_blank');
  }
}
