"""OmniTranslate0615Runner: full-language video translation pipeline.
...
class Omni0615LocalizationAdapter(_BaseAdapter):
...
class Omni0615JapaneseLocalizationAdapter(Omni0615LocalizationAdapter):
...
class Omni0615ModuleLocalizationAdapter(Omni0615LocalizationAdapter):
...
class OmniTranslate0615Runner(MultiTranslateRunner):
    """Multi-source-language video translation runner."""

    project_type: str = "omni_translate_0615"
    profile_code: str = "omni_0615"
    ...
    # 时 runner 是 OmniTranslate0615Runner，缺这些 helper 会 AttributeError）。
    # ------------------------------------------------------------------
    ...
    def pipeline_step_names_for_config(
        ...
        *,
        ...
        names.extend(["subtitle", "compose", "video_size_adjustment"])
        if include_analysis:
    ...
    # Thin shims dispatching to runtime_omni_0615_steps (5 个物理复制的算法体).
    # 这些方法 spec §6.2 要求暴露在 OmniTranslate0615Runner 上，便于 resume / 测试
    # 直接 ``runner._step_translate_standard(task_id)``。OmniProfile 也调它们。
    def _step_asr_normalize(self, task_id: str) -> None:
        from appcore import runtime_omni_0615_steps
        runtime_omni_0615_steps.step_asr_normalize(self, task_id)

    def _step_shot_decompose(self, task_id: str, video_path: str, task_dir: str) -> None:
        from appcore import runtime_omni_0615_steps
        runtime_omni_0615_steps.step_shot_decompose(self, task_id, video_path, task_dir)

    def _step_translate_standard(self, task_id: str, *, source_anchored: bool = True) -> None:
        from appcore import runtime_omni_0615_steps
        runtime_omni_0615_steps.step_translate_standard(
            self, task_id, source_anchored=source_anchored,
        )

    def _step_translate_shot_limit(self, task_id: str) -> None:
        from appcore import runtime_omni_0615_steps
        runtime_omni_0615_steps.step_translate_shot_limit(self, task_id)

    def _step_subtitle_asr_realign(self, task_id: str, task_dir: str) -> None:
        from appcore import runtime_omni_0615_steps
        runtime_omni_0615_steps.step_subtitle_asr_realign(self, task_id, task_dir)
    ...
    def _get_localization_module(self, task: dict):
        ...
        if lang == "ja":
            return Omni0615JapaneseLocalizationAdapter(
                source_language=source_language,
                original_asr_text=original_asr_text,
                product_context=product_context,
            )
        if lang in {"es", "it"}:
            return Omni0615ModuleLocalizationAdapter(
                lang=lang,
                source_language=source_language,
                original_asr_text=original_asr_text,
        ...
        return Omni0615LocalizationAdapter(
            lang=lang,
            source_language=source_language,
            original_asr_text=original_asr_text,
            product_context=product_context,
