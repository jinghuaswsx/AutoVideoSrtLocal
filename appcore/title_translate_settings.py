...
def _build_prompt(
    ...
    *,
    ...
    lines = [
        ...
        "",
        "输出格式",
        "严格输出三行，使用与输入完全一致的字段格式（字段名 + 半角冒号 + 空格 + 译文），示例：",
        f"标题: 这里填{lang_name}译文",
        f"文案: 这里填{lang_name}译文",
        f"描述: 这里填{lang_name}译文",
        "",
        "要求",
        "- 保留原意、关键信息和语气。",
    ...
    lines.extend(
        [
            f"- 必须将标题、文案、描述三段全部译为{lang_name}；即使原文看起来像品牌名、口号或单个英文短语，也必须给出{lang_name}译文，不允许在输出中保留任何英文单词或短语。",
