...
- 用户硬约束 "multi 不动"——本模块**物理复制**这些算法到 omni 命名空间，
  ``runtime_omni_0615.py`` 的 OmniTranslate0615Runner 上挂 thin shim method 调过来。
...
每个 ``_step_*`` 函数签名第一个参数是 ``runner``（OmniTranslate0615Runner 实例），
...
def step_translate_standard(runner, task_id: str, *, source_anchored: bool) -> None:
    ...
    - INPUT NOTICE 段: appcore/runtime_omni_0615.py:_step_translate（master 上当前 omni 默认）
    """
    task = task_state.get(task_id)
