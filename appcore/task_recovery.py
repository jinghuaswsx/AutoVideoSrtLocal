...
INTERRUPTED_PIPELINE_PROJECT_TYPES = {"multi_translate", "omni_translate", "translate_lab", "av_translate", "dialogue_translate"}
LINK_CHECK_RUNNING_STATUSES = {"locking_locale", "downloading", "analyzing", "summarizing"}
RECOVERABLE_PROJECT_TYPES = (
    {"video_creation", "video_review", "video_cover", "link_check", "image_translate", "subtitle_removal", "task_creator"}
...
# Backward-compatible aliases for older runtime probes/tests that read the
# active registry through task_recovery. New code should use appcore.active_tasks.
_active_tasks = active_tasks._active_tasks
_active_lock = active_tasks._active_lock
...
def recover_project_state(project_type: str, task_id: str, state: dict | None, active: bool | None = None) -> tuple[bool, dict, str | None]:
    ...
    elif project_type == "task_creator":
        changed = _mark_running_steps_as_error(recovered) or changed
