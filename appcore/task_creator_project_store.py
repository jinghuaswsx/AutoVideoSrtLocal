"""Database adapters for task creator (non-mk) projects."""
...
TASK_CREATOR_TYPE = "task_creator"
...
def list_projects(
    *,
    ...
    args: tuple = (TASK_CREATOR_TYPE,)
    if not is_admin:
        where += " AND p.user_id = %s"
        args = (TASK_CREATOR_TYPE, user_id)
    return query_func(
...
def get_project(
    ...
    *,
    ...
    args: tuple = (task_id, TASK_CREATOR_TYPE)
    if not is_admin:
        where += " AND p.user_id = %s"
        args = (task_id, TASK_CREATOR_TYPE, user_id)
