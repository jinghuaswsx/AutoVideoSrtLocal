...
def _build_prompt(*, target_language: str, target_language_name: str) -> str:
    return (
        ...
        "\n物理包装与固有文字英文豁免规则（非常重要）：\n"
        "- 我们允许并鼓励在非英语（如德语、法语等）图片中保留商品实物包装原本印刷的英文、机身物理丝印、产品固有标签（如说明书、配料成分表、CE/FCC/RoHS等认证标志、Model规格型号以及品牌专有名词LOGO等）。\n"
        "- 这些实物固有的物理英文不属于未翻译的营销宣传文本，不能因为它们而判定为“多语言混杂”或“翻译不合格”，这些实物固有英文的存在是合格的（应当保持原有，防止买家收到实物后产生“货不对版”的纠纷），必须判定为合格（decision=pass）。\n"
        "- 你的核心质检任务是检查图片中后期的营销文本、核心功能标注、大字标题宣传语、促销信息等电商宣传性质文案是否已被完美翻译并本土化为目标语种。只要营销宣传文案翻译语种正确且本地化合格，就应当通过（decision=pass）。\n"
        "\n输出要求：\n"
        "- 所有说明字段（text_summary、quality_reason）必须用简体中文回复，方便人工阅读。\n"
...
def _extract_payload(raw: object) -> dict:
    ...
    if isinstance(json_payload, dict):
        return json_payload
    if _PAYLOAD_KEYS.intersection(raw.keys()):
        return raw
    return {}
...
def analyze_image(image_path: str | Path, *, target_language: str, target_language_name: str) -> dict:
    ...
    raw = llm_client.invoke_generate(
        "link_check.analyze",
