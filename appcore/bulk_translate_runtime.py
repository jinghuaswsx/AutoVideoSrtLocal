...
- 串行创建子任务（支持延迟派发）
- 轮询子任务状态并回填结果
- 视频任务进入 AI 选音时按子任务实际状态同步，AI 排名完成后等待人工选音
- 失败恢复/重跑仍需手工触发
"""
...
def rebackfill_item(task_id: str, idx: int, user_id: int) -> None:
    task = get_task(task_id)
    if not task:
        raise ValueError(f"Task {task_id} not found")
    ...
    child_state = _load_child_snapshot(item.get("child_task_type"), child_task_id)
    if not child_state:
...
def _apply_child_snapshot(
    ...
    *,
    ...
    if _child_needs_voice(child_state):
        voice_gate_state = _child_voice_manual_gate_state(child_state)
        if voice_gate_state == "pending":
            item["status"] = "running"
            item["error"] = None
...
def _child_voice_manual_gate_state(child_state: dict) -> str:
