...
Only the translate/TTS/subtitle implementation is AV-sync specific.
"""
...
from appcore.events import EVT_SUBTITLE_READY, EVT_TRANSLATE_RESULT
...
class SentenceTranslateRunner(MultiTranslateRunner):
    ...
    def _step_translate(self, task_id: str) -> None:
        ...
        try:
            ...
            av_inputs = self._resolve_av_inputs(task)
            target_language = av_inputs["target_language"]
            target_language_name = self._target_language_name(av_inputs)

            _voice, _tts_voice_id, speech_rate_voice_id = self._resolve_av_voice(task)
            ...
            self._set_step(task_id, "translate", "running", "正在分析画面并生成时间轴笔记...")
            try:
                ...
                log.warning("sentence_translate shot notes unavailable for %s", task_id, exc_info=True)
                self._set_step(
                    task_id,
                    "translate",
                    "running",
            ...
            self._set_step(task_id, "translate", "running", "正在生成首版句级本土化译文...")
            ...
            self._set_step(task_id, "translate", "done", f"{target_language_name} 首版句级本土化完成")
        except Exception as exc:
            _fail_localize(task_id, self, current_step, str(exc))
    ...
    def _step_tts(self, task_id: str, task_dir: str) -> None:
        ...
        try:
            ...
            voice, tts_voice_id, _speech_rate_voice_id = self._resolve_av_voice(task)
            ...
            self._set_step(task_id, "tts", "running", "正在按句联合收敛文案与音频时长...")
            ...
            self._set_step(task_id, "tts", "done", f"{target_language_name} 配音收敛完成")
        except Exception as exc:
            _fail_localize(task_id, self, current_step, str(exc))
    ...
    def _step_subtitle(self, task_id: str, task_dir: str) -> None:
        ...
        try:
            ...
            self._set_step(task_id, "subtitle", "running", f"正在根据{target_language_name}句级音频生成字幕...")
            ...
            self._set_step(task_id, "subtitle", "done", f"{target_language_name} 字幕生成完成")
        except Exception as exc:
            _fail_localize(task_id, self, current_step, str(exc))
