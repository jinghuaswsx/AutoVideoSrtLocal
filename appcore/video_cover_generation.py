...
def _ad_copy_items(ad_copy_payload: dict[str, Any] | None) -> list[dict[str, Any]]:
    if not isinstance(ad_copy_payload, dict):
        return []
    items = ad_copy_payload.get("ad_copy_sets")
...
def _cover_hook(copy_item: dict[str, Any]) -> str:
    ...
    return str(english.get("headline") or "").strip()
    ...
    *,
    ...
    for index in range(1, image_count + 1):
        ...
        selected_copy_payload = {
            ...
            "all_ad_copy_sets": copy_payload.get("ad_copy_sets") if isinstance(copy_payload, dict) else [],
            "variant_index": index,
