...
CREATIVE_DIRECTOR_PROMPT_TEMPLATE = """请基于上传的产品图片、精选视频帧、cover_brief 和 selected_ad_copy，按 TikTok 封面 2K 竖版逻辑生成一张 9:16 竖版封面图，用于 Facebook Reels / Instagram Reels / TikTok / Shorts。
请像一位优秀的创意总监一样思考：目标是做出一张适合西方社交媒体、能提升短视频点击率的完整封面。
本地生图请求使用 1152x2048 的 2K 9:16 画布；最终交付验收尺寸必须是 1080x1920（宽度 x 高度），画面比例 9:16，格式为 PNG 或 JPG。
...
4. 必须把 selected_ad_copy.english.title 作为画面中唯一可读英文 hook。文字应由图片模型原生排版进画面，而不是留给后端叠加。

文字嵌入约束：
- 画面中必须且只能包含 selected_ad_copy.english.title 这一句可读英文 hook，大小写、单词和标点严格照抄。
- 不要新增品牌字样、字幕、UI、用户名、评论框、价格、折扣、按钮、标签、红圈、箭头、涂鸦或水印。
- hook 必须完整落在 9:16 安全区内，左右至少预留约 8% 画面宽度，上下避开 TikTok / Reels / Shorts 常见 UI 遮挡区；不得贴边、出血或被裁切。
- 如果 selected_ad_copy.english.title 较长，最多两行自然换行，并主动缩小字号、字重或字间距，保证每个单词完整可读。
- hook 要像真实短视频封面/达人编辑里的文字嵌入：根据画面主体和留白选择合适位置，可以在顶部、底部、角落或靠近主体，但不能遮挡产品关键功能和使用动作。
- 可以使用不同字体风格、字重、字号、描边、阴影、轻微倾斜或少量贴纸感强调，让每张候选图的文字排版有变化。
- 不要使用固定位置的半透明背景框、整条黑色横幅、模板化标题栏或大面积遮罩。只有当场景真实需要增强可读性时，才可用小范围自然阴影、描边或局部托底。
- 不要生成平台 UI、安全区辅助线、裁切框、尺寸标注或格式说明文字。
- 如果参考视频帧里有字幕、水印或界面残留，请忽略并移除，不要把它们复制到封面里。

视觉方向：
写实摄影风格，真实 UGC 质感，像 iPhone 15 Pro 拍摄，4K 清晰度，自然光，真实的西方生活方式，构图有吸引力但不过度设计。优先使用特写、中近景或中景，让产品成为主要视觉焦点之一。

最终任务：
生成一张强有力的 9:16 竖版封面图，在产品准确性、正确使用方式、高点击吸引力和自然 hook 嵌入之间取得平衡。"""
...
- title ≤ 42 个英文字符，优先 3 到 7 个英文单词，用作封面 hook 文本
...
- 英文文案默认要适量使用 emoji，5 组中默认 2 到 4 组带 emoji；只有产品调性严肃、敏感或 emoji 会削弱可信度时，才允许更少
- 每组最多 1 个 emoji，且只放在 english.title、english.message、english.description 三个字段之一
- 优先放在 english.message 的自然短语前后，或放在 english.title 的开头/结尾作为轻量 hook；description 只有自然时才用
- emoji 必须与产品品类、使用场景或具体利益点匹配，不要随机装饰
- 家居/收纳/厨房可用 ✨、✅、🏡、💡；车品/户外/旅行可用 🚗、✈️、⚡、✅；工具/维修/安装可用 🛠、✅、💪；宠物用品可用 🐾、✨、✅；礼品/家庭可用 🎁、🏡、✨
- 不要让 5 组都使用同一个 emoji，不要连续使用多个 emoji，不要把 emoji 当作列表项目符号
- 不得用 emoji 强化夸张承诺、医疗功效、恐吓、羞辱或身体缺陷暗示；如果会触发 Meta 风险，则不用 emoji
- 删除 emoji 后，英文仍必须完整、自然、可投放；中文翻译只忠实表达英文含义，不需要复制或解释 emoji
