...
CREATIVE_DIRECTOR_PROMPT_TEMPLATE = """请基于上传的产品图片、精选视频帧、{product_analysis}、{video_analysis} 和 {ad_copy_sets}，生成一张 9:16 竖版封面图，用于 Facebook Reels / Instagram Reels / TikTok / Shorts。
请像一位优秀的创意总监一样思考：目标是做出一张适合西方社交媒体、能提升短视频点击率的封面。
目标：
生成的封面应像真实爆款短视频中最值得停留的一帧。
它必须看起来自然、本土化、可信，并且在手机屏幕上一眼就能看懂。
...
- 它带来什么实际好处
请自然使用以下输入：
- product_image_*：最高优先级，用于保证产品外观和细节准确
- {product_analysis}：重点使用其中的 <产品核心理解>、<使用方式>、<外观与结构>，理解产品本质、使用逻辑、外观结构
- {video_analysis}：作为辅助参考，用来提取有价值的视频帧线索，例如动作时机、拍摄角度、手部位置、构图和真实感
- {ad_copy_sets}：作为画面文字 hook 方向的主要来源
创意自由：
请根据你对产品的判断，选择最适合这款产品、最容易被点击的封面方案。
你可以从以下方向中选择最清晰、最有吸引力的一种：
- 真实使用瞬间
- 结果导向画面
- 痛点被解决的瞬间
- 手部操作特写
...
- 在一个自然画面中呈现简单的前后对比线索
- 干净真实的生活方式使用场景
请选择最能清楚传达产品价值、同时最像西方短视频原生内容的方案。
不可妥协的要求：

1. 产品必须准确。

产品的形状、颜色、材质、表面质感、比例和可见功能部件，必须忠实于上传的产品图片。
不要把产品重新设计、美化或变成另一个物品。
2. 使用方式必须可信。
请根据 {product_analysis} 展示正确、自然的使用方式。
如果产品需要接触特定身体部位、物体、表面、安装位置或小范围目标区域，必须让这种互动准确且清楚可见。
3. 画面必须具有西方生活方式和社交平台原生感。
如果源素材中包含亚洲面孔、亚洲室内环境、中文文字或国内电商风格，请自然改写为西方生活方式场景，同时保留有用的产品逻辑。
4. 画面中必须且只能添加一句简短英文 hook。
请以 {ad_copy_sets} 为主要文案来源。
选择最强的一句短 hook，必要时可以改写得更短。
文案必须简洁、自然、适合手机阅读，并有助于提升点击。
推荐 hook 风格：
...
- CTA 按钮
文字处理：
把 hook 放在能辅助画面表达的位置，不要遮挡产品、关键动作或重要结果。
使用干净、加粗、易读的字体。
确保在手机屏幕上清晰可读。
让文字与画面自然融合，保持克制和有审美。
视觉方向：
写实摄影风格，真实 UGC 质感，像 iPhone 15 Pro 拍摄，4K 清晰度，自然光，真实的西方生活方式，构图有吸引力但不过度设计。
画面应该有生活气息、可信，而不是显得像精修广告。
构图指导：
优先使用特写、中近景或中景；除非广角画面明显更适合表达产品价值。
保持一个主要视觉想法。
让产品成为主要视觉焦点之一。
如果手部、身体互动、安装位置或可见结果能帮助解释产品，请清楚展示。
可以适当留白，以便文字放置和手机端阅读。
人物：
只有在人物能帮助用户理解产品或增强情绪连接时才加入人物。
很多情况下，只展示手部、局部身体或过肩视角就足够。
如果出现脸部，应真实自然，像普通用户，而不是精致广告模特。
请忽略视频帧中的以下内容：
- 运动模糊
- 压缩痕迹
- 界面残留
...
- 重度图形装饰
最终任务：
生成一张强有力的 9:16 竖版封面图，在产品准确性、正确使用方式和高点击吸引力之间取得平衡。
请在以上边界内做出有审美的创意判断。
画面中必须且只能包含一句简短英文 hook。
...
输出结构：
必须包裹在 <产品分析报告> 内，并包含以下小节：
<信息完整性检查>、<产品定义>、<核心功能>、<使用方式解析>、<物理特征>、<欧美本地化场景建议>、<视觉生成分类与封面策略>、<封面画面决策>、<广告文案方向>、<综合判断>。
...
输出要求：
请使用中文结构化文本，不要 JSON，不要 Markdown 表格。必须包含：
<视频素材分析>
video_text：
voiceover：
cover_reference：
...
AD_COPY_PROMPT_TEMPLATE = """角色：资深 Facebook / Instagram Reels 视频广告文案专家，熟悉欧美独立站 DTC 产品投放，擅长为短视频前 1–3 秒、视频正文、CTA 生成高点击且不夸张的英文广告文案。
...
- headline 必须短，适合视频封面或前 1 秒钩子
- body_text 必须适合短视频正文或广告主文案
- cta 必须自然，不要过度催促
...
- id：整数，范围 1–5
- angle：中文说明该组文案的测试角度
- english：对象，包含 headline、body_text、cta
- chinese_translation：对象，包含 headline、body_text、cta
  ...
  "ad_copy_sets": [
    {
      "id": 1,
      "angle": "痛点解决型",
      "english": {"headline": "", "body_text": "", "cta": ""},
      "chinese_translation": {"headline": "", "body_text": "", "cta": ""},
      "usage_note": ""
    },
    {
      "id": 2,
...
def _parse_ad_copy_response(response: dict[str, Any]) -> dict[str, Any]:
    ...
    ad_copy_sets = payload.get("ad_copy_sets")
    if not isinstance(ad_copy_sets, list) or len(ad_copy_sets) != 5:
        raise VideoCoverGenerationError("文案创作失败：ad_copy_sets 必须包含 5 组文案")
    required_top = {"id", "angle", "english", "chinese_translation", "usage_note"}
    required_text = {"headline", "body_text", "cta"}
    for idx, item in enumerate(ad_copy_sets, start=1):
        if not isinstance(item, dict) or not required_top <= set(item):
            raise VideoCoverGenerationError(f"文案创作失败：第 {idx} 组字段不完整")
        if not isinstance(item.get("english"), dict) or not required_text <= set(item["english"]):
            raise VideoCoverGenerationError(f"文案创作失败：第 {idx} 组 english 字段不完整")
        if (
            not isinstance(item.get("chinese_translation"), dict)
            or not required_text <= set(item["chinese_translation"])
        ):
            raise VideoCoverGenerationError(f"文案创作失败：第 {idx} 组 chinese_translation 字段不完整")
...
def generate_video_covers(
    *,
    ...
    ad_copy_sets = json.dumps(copy_payload, ensure_ascii=False, indent=2)
    covers = []
    for spec in PLATFORM_SPECS:
        ...
        key = _write_png_artifact(user_id, task_id, f"{spec.platform}.png", png)
        covers.append(
            {
