...
_EVAL_PROMPT = (
    ...
    "请重点检查并回答以下问题：\n"
    "1. 语言一致性与英文免检规则（极其重要）：\n"
    "   - 我们允许且鼓励目标语言图中保留产品实物本身固有印刷英文、瓶身/机身物理丝印、产品固有标签/成分配料表、Model规格型号、认证标志（如CE/FCC等）、以及品牌LOGO专有名词。\n"
    "   - 只有产品本身印刷的英文可以不翻译、不更换，以便确保买家收到的货和视频/图片素材对版（防范“货不对版”纠纷）。这些产品实物固有英文的存在，绝对不能判定为“多语言混杂”或“翻译不合格”，不可因此扣分或报错。\n"
    "   - 你的检查重点是：图片后期的非实物营销性文案、核心卖点宣传语、大字标题、功能标注、促销折扣等说明性文字，是否已被完全翻译为目标语种 {target_lang_name}。如果所有营销性文案和功能标注均已正确翻译，且无非固有的英文残留，即可判定为合格并给予最高评分。\n"
    "2. 翻译准确度与地道度：目标语言营销文本的翻译是否准确、地道、通顺且符合目标市场商业习惯？是否存在生硬机翻或严重语法/拼写错误？翻译质量是评分的唯一决定性因素。\n"
    "3. 排版与布局完全免检说明（绝对硬红线）：\n"
    "   - 我们完全不考核、不评估任何排版布局、文字重叠、溢出、遮挡、字体大小或任何视觉排版设计问题。排版这些只要跟原图一致就完全没问题，不影响任何评分。\n"
    "   - 请你在此字段始终将 has_layout_issue 设为 false，并将 layout_issue_details 设为空字符串 \"\"。决不能因为排版、字体、视觉或布局问题在 issues 中有任何提及或报错，也绝不能因此扣减评分。\n\n"
    "所有面向人工阅读的字段（包括 summary、issues、mixed_languages_details、layout_issue_details）必须使用简体中文进行回答。\n"
    "translation_quality_score 的范围为 1 至 10（整数），且翻译质量是唯一决定该评分的因素。"
)
