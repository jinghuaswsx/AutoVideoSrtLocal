...
_EVAL_PROMPT = (
    ...
    "3. 排版视觉效果说明（极其重要）：我们完全不评估任何排版布局、文字重叠、溢出、遮挡、字体大小或排版美观度（因为翻译保持原图排版即可，原图怎样我们就怎样）。请你在此字段始终将 has_layout_issue 设为 false，并将 layout_issue_details 设为空字符串 \"\"。决不能因为排版视觉效果或布局问题而在 issues 中报错，更不能因此扣减 translation_quality_score（质量分必须只取决于翻译和文字本身）。\n\n"
