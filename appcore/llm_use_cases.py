...
USE_CASES: dict[str, UseCase] = {
    ...
    "video_translate.localize": _uc(
        ...
        "gemini_vertex", "gemini-3.1-flash-lite-preview", "gemini_vertex", "tokens",
    ),
    ...
    "video_translate.tts": _uc(
        ...
        "ElevenLabs 生成本土化配音",
        "elevenlabs", "<runtime>", "elevenlabs", "chars",
    ),
    "video_translate.asr": _uc(
        "video_translate.asr", "video_translate", "ASR 识别",
        "豆包语音识别原视频",
        "doubao_asr", "big-model", "doubao_asr", "seconds",
    ),
    # ── 文案创作 ──
    "copywriting.generate": _uc(
        "copywriting.generate", "copywriting", "文案生成",
        "根据关键帧+商品信息生成带货文案",
        "openrouter", "anthropic/claude-sonnet-4.6", "openrouter", "tokens",
    ),
    ...
    "copywriting_translate.generate": _uc(
        "copywriting_translate.generate", "copywriting", "文案翻译",
        "把英文带货文案翻译成目标语种",
        "openrouter", "anthropic/claude-sonnet-4.6", "openrouter", "tokens",
    ),
    ...
    "video_score.run": _uc(
        ...
        "对硬字幕成片按美国带货要素打分",
        "gemini_aistudio", "gemini-3.1-pro-preview", "gemini_video_analysis", "tokens",
    ),
    ...
    "image_translate.detect": _uc(
        ...
        "判定商品图是否已本地化为目标语种",
        "gemini_aistudio", "gemini-2.5-flash", "gemini", "tokens",
    ),
    "image_translate.generate": _uc(
        "image_translate.generate", "image", "图片本地化重绘",
        "用图像模型重绘目标语种的商品图",
        "gemini_aistudio", "gemini-3-pro-image-preview", "gemini", "images",
    ),
    ...
    "link_check.same_image": _uc(
        "link_check.same_image", "image", "同图判断",
        "比较抓取图与参考图是否属于同一基础图片",
        "gemini_aistudio", "gemini-3.1-flash-lite-preview", "gemini", "tokens",
    ),
