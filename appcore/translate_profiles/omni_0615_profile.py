...
``plugin_config`` 把每步 dispatch 到对应算法实现：
    ...
    translate → runner._step_translate_standard / _step_translate_shot_limit /
                AvSync0615Profile.translate (av_sentence)
    tts       → sentence_reconcile/_v2 → 0615 strategy 直接 import；其余 → get_strategy()
    subtitle  → runner._step_subtitle_asr_realign /
                AvSync0615Profile.subtitle (sentence_units)
...
def _resolve_cfg(runner) -> dict | None:
    ...
    Omni0615Profile.X 都接收 task_id 作为入参。这里通过 runner._resolve_plugin_config
    走兜底逻辑。
    """
    return None  # 实际读取在 dispatch 时按 task_id 做


class Omni0615Profile(TranslateProfile):
    code = "omni_0615"
    name = "全能视频翻译0615"
    ...
    def translate(self, runner: "PipelineRunner", task_id: str) -> None:
        ...
        if algo == "standard":
            ...
            # 直接复用 AvSync0615Profile.translate（不需要物理复制——本来就是为
            # omni 抽象设计的 profile，不是另一套 production runtime）。
            from .av_sync_0615_profile import AvSync0615Profile
            AvSync0615Profile().translate(runner, task_id)
        else:
    ...
    def tts(self, runner: "PipelineRunner", task_id: str, task_dir: str) -> None:
        cfg = runner._resolve_plugin_config(task_id)
        ts = cfg["tts_strategy"]
        if ts == "sentence_reconcile":
            from appcore.tts_strategies.sentence_reconcile_0615 import SentenceReconcile0615Strategy
            strategy = SentenceReconcile0615Strategy()
        elif ts == "sentence_reconcile_v2":
            from appcore.tts_strategies.sentence_reconcile_0615_v2 import SentenceReconcile0615StrategyV2
            strategy = SentenceReconcile0615StrategyV2()
        else:
            from appcore.tts_strategies import get_strategy
            strategy = get_strategy(ts)
    ...
    def subtitle(self, runner: "PipelineRunner", task_id: str, task_dir: str) -> None:
        ...
        if cfg["subtitle"] == "asr_realign":
            ...
            from .av_sync_0615_profile import AvSync0615Profile
            AvSync0615Profile().subtitle(runner, task_id, task_dir)
        else:
