...
class AvSync0615Profile(TranslateProfile):
    ...
    # av_sync 走句级 reconcile 策略（PR6）。
    tts_strategy_code = "sentence_reconcile_0615"
    ...
    def get_tts_strategy(self):
        # 0615 隔离：直接返回 0615 句级 strategy 实例，不经全局注册表
        # （0615 strategy 故意不注册）。
        from appcore.tts_strategies.sentence_reconcile_0615 import SentenceReconcile0615Strategy
        return SentenceReconcile0615Strategy()

    def tts(self, runner: "PipelineRunner", task_id: str, task_dir: str) -> None:
        # 0615 隔离：直接 import 0615 句级 strategy，不走全局注册表。
        from appcore.tts_strategies.sentence_reconcile_0615 import SentenceReconcile0615Strategy
        SentenceReconcile0615Strategy().run(runner, self, task_id, task_dir)
