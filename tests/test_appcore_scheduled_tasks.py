...
def test_task_definitions_include_two_hour_dianxiaomi_sku_and_purchase_sync():
    ...
    assert yuncang_task["schedule"] == "每 2 小时（01:03 起，错开 SKU 同步）"
    ...
    assert "xmyc_storage_sync" not in definitions
...
def test_task_definitions_include_push_quality_check():
    ...
    assert task["schedule"] == "持续后台任务池（5并发）"
    assert "5 并发线程池持续扫描" in task["description"]
    assert "守护哨兵" in task["description"]
...
def test_task_definitions_include_tabcut_video_translation_task():
    ...
    assert task["log_table"] == "scheduled_task_runs"
    assert "250 个视频" in task["description"]
    assert "系统设置" in task["description"]
    assert "Gemini 3.1 Flash Lite" in task["description"]
...
def test_task_definitions_include_server_and_app_timers():
    ...
    assert "每 1 小时" not in definitions["dianxiaomi_order_import"]["description"]
    assert ":02" not in definitions["dianxiaomi_order_import"]["description"]
    assert "每 1 小时" not in definitions["meta_realtime_import"]["description"]
    assert ":02" not in definitions["meta_realtime_import"]["description"]
    ...
    assert definitions["meta_daily_final"]["runner"] == "tools/meta_daily_final_sync.py --mode run --include-adsets"
    ...
    assert (
        definitions["ad_order_previous_business_day_sync"]["runner"]
        == "tools/ad_order_sync_orchestrator.py --mode previous-business-day"
    )
    ...
    assert (
        definitions["ad_order_previous_week_sync"]["runner"]
        == "tools/ad_order_sync_orchestrator.py --mode previous-week --max-scan-pages 500"
    )
    ...
    assert definitions["task_center_auto_archive"]["schedule"] == "每小时"
    ...
    assert "2026-06-08-task-center-unarchive-hourly-archive-design.md" in definitions["task_center_auto_archive"]["description"]
