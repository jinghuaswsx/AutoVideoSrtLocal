...
def test_xuanpin_mk_page_uses_xuanpin_tabs_and_api(authed_client_no_db):
    ...
    assert 'id="mkUploaderFilter"' in body
    assert 'placeholder="上传者"' in body
    assert "mkUploaderQueryParam()" in body
...
def test_xuanpin_mk_material_library_api_reads_local_archive(
    ...
    resp = authed_client_no_db.get(
        ...
        "&library_status=video_not_imported&ad_delivery=all&uploader=Alice"
    )
