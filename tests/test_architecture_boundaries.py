...
def test_video_creation_route_db_dependencies_use_appcore_store():
    route_source = Path("web/routes/video_creation.py").read_text(encoding="utf-8")
    store_path = Path("appcore/video_creation_route_store.py")

    assert "from appcore.db import" not in route_source
    assert "video_creation_route_store" in route_source
    assert "db_query = video_creation_route_store.query" in route_source
    assert "db_query_one = video_creation_route_store.query_one" in route_source
    assert "db_execute = video_creation_route_store.execute" in route_source
...
def test_media_products_list_response_lives_outside_route_module():
    ...
    assert "list_yuncang_unit_prices" not in route_source
...
def test_text_translate_project_sql_lives_in_appcore_store():
    route_source = Path("web/routes/text_translate.py").read_text(encoding="utf-8")
    store_source = Path("appcore/text_translate_store.py").read_text(encoding="utf-8")
    ...
    for helper_name in [
        ...
        assert f"def {helper_name}" in store_source
        assert f"text_translate_store.{helper_name}" in route_source
...
def test_removed_xmyc_sku_routes_are_not_registered():
    module_source = Path("web/routes/medias/products.py").read_text(encoding="utf-8")
    module = ast.parse(module_source)
    for function_name in (
        "api_list_xmyc_skus",
    ...
    assert "_build_xmyc_skus_list_response" not in module_source
    assert "media_xmyc_skus" not in module_source
    assert not Path("web/services/media_xmyc_skus.py").exists()
...
def test_refresh_shopify_sku_response_lives_outside_route_module():
    ...
    assert "medias.list_yuncang_unit_prices" not in route_source
...
def test_de_fr_translate_route_db_dependencies_use_appcore_store():
...
def test_ja_translate_route_db_dependencies_use_appcore_store():
    source = Path("web/routes/ja_translate.py").read_text(encoding="utf-8")
    store_path = Path("appcore/translation_route_store.py")
...
def test_ja_translate_project_sql_lives_in_appcore_store():
    route_source = Path("web/routes/ja_translate.py").read_text(encoding="utf-8")
    store_source = Path("appcore/translation_route_store.py").read_text(encoding="utf-8")
    helper_names = [
        "find_project_by_display_name",
        "set_project_thumbnail_path",
...
def test_multi_translate_route_db_dependencies_use_appcore_store():
    source = Path("web/routes/multi_translate.py").read_text(encoding="utf-8")
    store_path = Path("appcore/translation_route_store.py")
...
def test_tasks_list_query_lives_in_appcore_tasks():
    module_source = Path("web/routes/tasks.py").read_text(encoding="utf-8")
    module = ast.parse(module_source)
    route_function = next(
        node
        for node in module.body
        if isinstance(node, ast.FunctionDef) and node.name == "api_list"
    )
    route_source = ast.get_source_segment(module_source, route_function) or ""
    appcore_source = Path("appcore/tasks.py").read_text(encoding="utf-8")
    ...
    assert "tasks_svc.high_level_status" not in route_source
    assert "tasks_svc.list_task_center_items" in route_source
...
def test_task_upload_rename_and_delete_db_access_lives_in_appcore_project_state():
    ...
    delete_source = Path("web/services/task_deletion.py").read_text(encoding="utf-8")
    ...
    assert "from appcore.db import" not in delete_source
    ...
    assert "UPDATE projects SET deleted_at=%s WHERE id=%s" not in delete_source
    assert "SELECT id, task_dir, state_json FROM projects" not in delete_source
    ...
    assert "get_project_delete_row" in delete_source
    assert "soft_delete_project" in delete_source
    ...
    assert "def get_project_delete_row" in dao_source
    assert "def soft_delete_project" in dao_source
