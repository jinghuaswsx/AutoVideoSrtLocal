...
def test_cache_card_video_forces_local_mingkong_download(monkeypatch):
    ...
    from appcore import mingkong_fine_ai_auto_evaluation as mod
    from appcore import mk_media_helpers
    ...
    captured = {}
    monkeypatch.setattr(mk_media_helpers, "normalize_mk_media_path", lambda value: "uploads2/video-1.mp4")
    monkeypatch.setattr(local_media_storage, "safe_local_path_for", lambda object_key: MissingLocalPath())
    monkeypatch.setattr(mk_media_helpers, "build_mk_video_cache_object_key", lambda media_path, cache_prefix: "mk/videos/video-1.mp4")
    monkeypatch.setattr(mk_media_helpers, "build_mk_request_headers", lambda: {"Authorization": "Bearer token"})
    monkeypatch.setattr(mk_media_helpers, "get_mk_api_base_url", lambda: "https://mk.example")
    
    fake_http_get = object()
    monkeypatch.setattr(mk_media_helpers, "mk_http_get", fake_http_get)
    ...
    monkeypatch.setattr(mk_media_helpers, "cache_mk_video", fake_cache_impl)
    ...
    assert captured["http_get_fn"] is fake_http_get
