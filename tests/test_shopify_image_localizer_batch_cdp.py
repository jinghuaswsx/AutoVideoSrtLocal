...
from tools.shopify_image_localizer import domain_image_mapping
...
def test_pair_carousel_images_uses_domain_alias_source_index_when_target_token_differs():
    ...
    target_product = {
        "images": [
            "https://cdn.shopify.com/s/files/1/other/files/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb.jpg?v=2",
        ],
        "description": "",
    }
    ...
    localized_images = [
        _localized("loc_from_url_en_00_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.png"),
    ]

    pairs = run_product_cdp.pair_carousel_images(
        localized_images,
    ...
    assert pairs == [
        (0, str(Path("C:/tmp") / "loc_from_url_en_00_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.png")),
    ]
...
def test_detail_plan_uses_domain_alias_source_index_when_token_differs():
    target_src = "https://cdn.shopify.com/s/files/1/other/files/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb.jpg?v=2"
    html = f'<section><img src="{target_src}" alt="Product detail"></section>'
    mapping = domain_image_mapping.DomainImageMapping(
        target_domain="omurio.com",
        canonical_domain="newjoyloo.com",
        detail_source_index_by_key={
            "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb": 8,
        },
    )
    localized_images = [
        _localized("loc_from_url_en_08_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.png"),
    ]

    plan = taa_cdp.plan_body_html_replacements(
        html,
        localized_images,
        source_index_by_token=mapping.detail_source_index_by_key,
        replace_shopify_cdn=True,
    )

    assert len(plan["replacements"]) == 1
    assert plan["replacements"][0]["match_method"] == "source_index"
    assert plan["replacements"][0]["candidate"]["local_path"] == str(
        Path("C:/tmp") / "loc_from_url_en_08_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.png"
    )
...
def test_normalize_detail_candidate_sizes_resizes_to_reference_token(tmp_path):
    token = "fde938b6b7f3a20badb4752d1dd70324"
    local_path = tmp_path / f"loc_from_url_en_08_{token}.png"
    Image.new("RGB", (1254, 1254), "white").save(local_path)
    rows = [{
        "kind": "detail",
...
def test_controller_passes_gui_shopify_id_to_batch_runner(monkeypatch):
    ...
    assert saved_config[0]["base_url"] == "https://autovideosrt.example.test"
