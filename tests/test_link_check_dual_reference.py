...
def test_dual_reference_match_heuristic_detects_non_replaced(tmp_path, monkeypatch):
    ...
    # Mock judge_same_image and analyze_image to avoid network calls (raising=False since they may be refactored/skipped in runtime)
    monkeypatch.setattr("appcore.link_check_runtime.judge_same_image", lambda a, b: {"status": "done", "answer": "否"}, raising=False)
    monkeypatch.setattr(
...
def test_dual_reference_no_text_bypass(tmp_path, monkeypatch):
    # 1. Prepare three dimensions of images where original and translated are virtually identical (no text)
    # O (Original English - no text)
    path_orig = _make_image(tmp_path / "original_en.jpg", "")
    # T (Translated target - no text)
    path_ref = _make_image(tmp_path / "translated_fr.jpg", "")
    # C (Crawled page image - no text)
    path_crawled_en = _make_image(tmp_path / "crawled_storefront.jpg", "")

    # Mock judge_same_image and analyze_image to avoid network calls (raising=False since they may be refactored/skipped in runtime)
    monkeypatch.setattr("appcore.link_check_runtime.judge_same_image", lambda a, b: {"status": "done", "answer": "是"}, raising=False)
    monkeypatch.setattr(
        "appcore.link_check_runtime.analyze_image",
        lambda a, **kwargs: {"decision": "pass", "quality_score": 100, "quality_reason": "Audit OK"},
        raising=False
    ...
    # 4. Verify that since they are virtually identical (difference < 0.02), we bypassed the dual-reference override
    # is_replaced should be True, and decision should be "pass"
    assert result["is_replaced"] is True
    assert result["analysis"]["decision"] == "pass"
    assert "绿色免检通道" in result["analysis"]["text_summary"]
