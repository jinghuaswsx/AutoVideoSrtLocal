...
def test_order_analytics_weekly_ai_analysis_use_case_registered():
    ...
    product_uc = USE_CASES["order_analytics.weekly_product_action_evaluation"]
    assert product_uc["module"] == "order_analytics"
    assert product_uc["default_provider"] == "openrouter"
    assert product_uc["default_model"] == "google/gemini-3.5-flash"
    assert product_uc["usage_log_service"] == "openrouter"
    assert product_uc["units_type"] == "tokens"


def test_ad_alert_evaluate_use_case_registered():
    uc = USE_CASES["ad_alert.evaluate"]

    assert uc["module"] == "ad_alerts"
    assert uc["label"] == "广告预警 AI 评估"
    assert uc["default_provider"] == "openrouter"
    assert uc["default_model"] == "google/gemini-3.5-flash"
    assert uc["usage_log_service"] == "openrouter"
    assert uc["units_type"] == "tokens"
    assert MODULE_LABELS["ad_alerts"] == "广告预警"
...
def test_meta_hot_posts_translate_product_title_use_case_is_registered_for_billing():
    uc = USE_CASES["meta_hot_posts.translate_product_title"]
...
def test_meta_hot_posts_video_copyability_use_case_is_registered_for_billing():
