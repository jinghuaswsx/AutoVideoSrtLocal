...
def test_from_url_response_auto_clears_existing_images_when_frontend_state_is_stale():
    from web.services.media_detail_from_url import build_detail_images_from_url_response
    ...
    def create_fetch_task_fn(*, user_id, product_id, url, lang, worker):
        ...
        worker("mdf-stale-state", update)
        return "mdf-stale-state"
    ...
    result = build_detail_images_from_url_response(
        123,
        7,
        {"product_code": "floating-lint-catcher-rjc"},
        {
            "lang": "en",
            "url": "https://newjoyloo.com/products/floating-lint-catcher-rjc",
            "clear_existing": False,
        },
        is_valid_language_fn=lambda code: code == "en",
        create_fetch_task_fn=create_fetch_task_fn,
        fetch_page_fn=lambda url, lang: SimpleNamespace(
            images=[{"source_url": "https://cdn.example.test/from-url-1.jpg"}]
        ),
        download_image_to_local_media_fn=lambda url, pid, prefix, *, user_id=None: (
            calls.append(("download", url, pid, prefix, user_id))
            or ("7/medias/123/from-url-1.jpg", b"image-bytes", ".jpg")
        ),
        soft_delete_detail_images_by_lang_fn=lambda product_id, lang: calls.append(("clear", product_id, lang)) or 16,
        detail_image_empty_counts_fn=lambda: calls.append(("empty-counts",)) or {"image": 0, "gif": 0},
        detail_image_existing_counts_fn=lambda product_id, lang: (
            calls.append(("existing-counts", product_id, lang)) or {"image": 16, "gif": 0}
        ),
        detail_image_kind_from_download_ext_fn=lambda ext: "gif" if ext == ".gif" else "image",
        detail_image_limits={"image": 20, "gif": 1},
        detail_image_kind_labels={"image": "image", "gif": "GIF"},
        add_detail_image_fn=add_detail_image_fn,
        get_detail_image_fn=lambda image_id: {
            "id": image_id,
            ...
            "lang": "en",
            "sort_order": 0,
            "object_key": "7/medias/123/from-url-1.jpg",
            "content_type": "image/jpeg",
            "file_size": 11,
            "width": None,
            "height": None,
    ...
    assert result.status_code == 202
    assert calls == [
        ("create", 7, 123, "https://newjoyloo.com/products/floating-lint-catcher-rjc", "en"),
        ("existing-counts", 123, "en"),
        ("clear", 123, "en"),
        ("empty-counts",),
        ("download", "https://cdn.example.test/from-url-1.jpg", 123, "from_url_en_00", 7),
        (
            "add",
            123,
            "en",
            "7/medias/123/from-url-1.jpg",
            {"content_type": None, "file_size": 11, "origin_type": "from_url"},
        ),
    ]
    assert worker_state["status"] == "done"
    assert worker_state["inserted"] == [{"id": 601, "object_key": "7/medias/123/from-url-1.jpg"}]
    assert worker_state["errors"] == []
