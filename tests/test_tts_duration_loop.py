...
class TestSegmentCandidateAssembly:
    ...
    def test_adaptive_speed_starts_from_measured_ratio_and_supports_slowdown(self):
        ...
        assert _adaptive_speed_candidate(
            base_duration=61.5, video_duration=60.0,
            previous_candidates=[],
        ) == pytest.approx(1.03)
        assert _adaptive_speed_candidate(
            base_duration=58.5, video_duration=60.0,
            previous_candidates=[],
        ) == pytest.approx(0.97)
        assert _adaptive_speed_candidate(
            base_duration=59.5, video_duration=60.0,
            previous_candidates=[],
        ) is None

    def test_adaptive_speed_uses_feedback_and_never_leaves_quality_bounds(self):
        from appcore.runtime import _adaptive_speed_candidate

        assert _adaptive_speed_candidate(
            base_duration=61.5, video_duration=60.0,
            previous_candidates=[{"speed": 1.03, "duration": 60.8}],
        ) == pytest.approx(1.04)
        assert _adaptive_speed_candidate(
            base_duration=58.5, video_duration=60.0,
            previous_candidates=[{"speed": 0.97, "duration": 58.8}],
        ) == pytest.approx(0.96)
        assert _adaptive_speed_candidate(
    ...
    def test_selects_closest_under_video_segment_combination(self):
        ...
        result = _select_segment_candidate_assembly(
            [
                [
                    {"segment_index": 0, "duration": 20.8, "source": "round", "speed": 1.0},
                    {"segment_index": 0, "duration": 20.0, "source": "speedup", "speed": 1.04},
                ],
                [
                    {"segment_index": 1, "duration": 20.4, "source": "round", "speed": 1.0},
                    {"segment_index": 1, "duration": 20.0, "source": "speedup", "speed": 1.04},
                ],
                [
                    {"segment_index": 2, "duration": 20.3, "source": "round", "speed": 1.0},
                    {"segment_index": 2, "duration": 19.9, "source": "speedup", "speed": 1.04},
                ],
            ],
            video_duration=60.0,
        )
        ...
        assert result["total_duration"] == pytest.approx(59.9)
        assert [c["source"] for c in result["selected"]] == [
            "speedup", "speedup", "speedup",
        ]

    def test_rejects_combinations_that_still_exceed_video(self):
        from appcore.runtime import _select_segment_candidate_assembly
        ...
        result = _select_segment_candidate_assembly(
            [
                [
                    {"segment_index": 0, "duration": 20.8, "source": "round", "speed": 1.0},
                    {"segment_index": 0, "duration": 20.4, "source": "speedup", "speed": 1.03},
                ],
                [
                    {"segment_index": 1, "duration": 20.4, "source": "round", "speed": 1.0},
                    {"segment_index": 1, "duration": 20.2, "source": "speedup", "speed": 1.03},
                ],
                [
                    {"segment_index": 2, "duration": 20.3, "source": "round", "speed": 1.0},
                    {"segment_index": 2, "duration": 20.1, "source": "speedup", "speed": 1.03},
                ],
            ],
            video_duration=59.0,
        )
        ...
        assert result["best_under_duration"] is None
        assert result["candidate_combination_count"] == 8
        assert result["min_duration"] == pytest.approx(60.7)
        assert result["closest_over_duration"] == pytest.approx(60.7)
        assert [c["duration"] for c in result["closest_over_selected"]] == [
            pytest.approx(20.4), pytest.approx(20.2), pytest.approx(20.1),
        ]

    def test_tie_prefers_fewer_speed_modified_segments(self):
        from appcore.runtime import _select_segment_candidate_assembly

        result = _select_segment_candidate_assembly(
            [
                [
                    {"segment_index": 0, "duration": 20.0, "source": "round", "speed": 1.0},
                    {"segment_index": 0, "duration": 19.5, "source": "speedup", "speed": 1.04},
                ],
                [
                    {"segment_index": 1, "duration": 20.0, "source": "round", "speed": 1.0},
                    {"segment_index": 1, "duration": 20.5, "source": "speedup", "speed": 1.04},
                ],
                [
                    {"segment_index": 2, "duration": 19.0, "source": "round", "speed": 1.0},
                ],
            ],
            video_duration=59.0,
        )
        ...
        assert result["hit"] is True
        assert result["total_duration"] == pytest.approx(59.0)
        assert [c["source"] for c in result["selected"]] == [
            "round", "round", "round",
        ]

    def test_exact_search_finds_hit_that_small_beam_would_prune(self):
        from appcore.runtime import _select_segment_candidate_assembly

        result = _select_segment_candidate_assembly(
            [
                [
                    {"segment_index": 0, "duration": 70.0, "source": "round", "speed": 1.0},
                    {"segment_index": 0, "duration": 50.0, "source": "speedup", "speed": 1.04},
                ],
                [
                    {"segment_index": 1, "duration": 20.0, "source": "round", "speed": 1.0},
                ],
                [
                    {"segment_index": 2, "duration": 29.0, "source": "round", "speed": 1.0},
                ],
            ],
        ...
        assert result["hit"] is True
        assert result["total_duration"] == pytest.approx(99.0)
        assert [c["duration"] for c in result["selected"]] == [50.0, 20.0, 29.0]
...
class TestDurationLoopRound1Only:
    ...
    def test_round1_emits_substep_msgs_for_each_phase(self, tmp_path, monkeypatch):
        """Round 1 应该发出多条 substep msg 覆盖整个流程，避免任何 5s+ 的静默期。"""
        runner = self._make_runner()
        from appcore import task_state
    ...
    def test_audio_segment_progress_is_persisted_for_polling(self, tmp_path, monkeypatch):
        """Polling clients should see the same TTS segment progress as websocket clients."""
        runner = self._make_runner()
        from appcore import task_state
        ...
        task_state.create("polling-progress-task", "v.mp4", str(tmp_path),
                          original_filename="v.mp4")

        import importlib
        loc_mod = importlib.import_module("pipeline.localization")
        runner._run_tts_duration_loop(
            task_id="polling-progress-task", task_dir=str(tmp_path),
            loc_mod=loc_mod, provider="openrouter",
            video_duration=30.0, voice={"elevenlabs_voice_id": "v1"},
            initial_localized_translation={"full_text": "hi.", "sentences": [{"text": "hi."}]},
            source_full_text="hi.", source_language="en",
        ...
        assert snapshots, "segment callback did not capture an intermediate state"
        assert "1/2" in snapshots[0]["message"]
        assert snapshots[0]["rounds"]
        assert snapshots[0]["rounds"][0]["audio_segments_total"] == 2
        assert snapshots[0]["rounds"][0]["audio_segments_done"] == 1
        assert snapshots[0]["rounds"][0]["__current_phase"] == "audio_gen"
...
class TestDurationLoopMultiRound:
    def _setup(self, monkeypatch, tmp_path, audio_durations):
        ...
        def fake_regenerate_with_speed(tts_segments, voice_id, task_dir, variant=None, **kw):
            out = os.path.join(task_dir, f"tts_full.{variant}.speedup.mp3")
            with open(out, "wb") as f:
                f.write(b"fake-speedup")
            return {"full_audio_path": out,
                    "segments": [{"index": 0, "tts_path": out, "tts_duration": audio_durations[-1]}]}

        def fake_assemble(selected, task_dir, variant):
            out = os.path.join(task_dir, f"tts_full.{variant}.assembled.mp3")
            with open(out, "wb") as f:
                f.write(b"fake-assembled")
            segments = []
            for i, seg in enumerate(selected):
                seg_copy = dict(seg)
                seg_copy.setdefault("index", i)
                seg_copy["tts_duration"] = seg_copy.get(
                    "tts_duration", seg_copy.get("duration", 0.0)
                )
                segments.append(seg_copy)
            return {"full_audio_path": out, "segments": segments}

        def fake_get_audio_duration(path):
            if str(path).endswith(".speedup.mp3"):
                return audio_durations[-1]
            idx = call_counter["i"]
            call_counter["i"] += 1
        ...
        monkeypatch.setattr("pipeline.tts.generate_full_audio", fake_gen_full_audio)
        monkeypatch.setattr("pipeline.tts.regenerate_full_audio_with_speed", fake_regenerate_with_speed)
        monkeypatch.setattr("pipeline.tts.assemble_full_audio_from_segments", fake_assemble)
        monkeypatch.setattr("pipeline.tts._get_audio_duration", fake_get_audio_duration)
        monkeypatch.setattr("pipeline.translate.generate_tts_script", fake_gen_tts_script)
    ...
    def test_round1_audio_below_video_minus_one_runs_stage1_speedup(self, tmp_path, monkeypatch):
        ...
        assert result["rounds"][0]["speedup_context"] == "stage1_converged_postprocess"
        assert result["rounds"][0]["segment_assembly_duration"] == pytest.approx(29.0)

    def test_round2_stage1_hit_runs_postprocess_and_finishes(self, tmp_path, monkeypatch):
        # video=35
        # old stage1 range: [31.5, 38.5]
        # final range: [34.0, 37.0]
        # round 2 audio=38.2 now stops rewrite and enters speedup post-process.
        ...
        assert result["rounds"][1]["segment_assembly_duration"] == pytest.approx(34.0)
...
class TestSpeedupShortcut:
    ...
    def _common_patches(self, monkeypatch, audio_dur, speedup_dur=None,
        ...
        monkeypatch.setattr(
            ...
            lambda **kw: {
                "full_text": " ".join(["x"] * int(kw.get("target_words") or 3)),
                "sentences": [{"text": "x"}],
                "_usage": {},
                "_messages": [],
            },
        ...
        duration_by_path = {}
        regular_audio_durs = list(audio_dur) if isinstance(audio_dur, list) else [audio_dur]
        regular_call = {"i": 0}

        def _segment_durations_for_regular_round(current_dur):
            if audio_segments is not None:
                return list(audio_segments)
            return [current_dur]

        # generate_full_audio：写一个空 mp3，segments 占位
        def fake_full_audio(segs, voice_id, task_dir, variant=None, **kw):
            idx = min(regular_call["i"], len(regular_audio_durs) - 1)
            current_dur = regular_audio_durs[idx]
            regular_call["i"] += 1
            out = os.path.join(task_dir, f"tts_full.{variant}.mp3")
            with open(out, "wb") as f:
                f.write(b"\xff\xfb\x10\x00")
            duration_by_path[out] = current_dur
            seg_durations = _segment_durations_for_regular_round(current_dur)
            segment_rows = []
            for i, seg_duration in enumerate(seg_durations):
                seg_path = os.path.join(
                    task_dir, "tts_segments", str(variant), f"seg_{i:04d}.mp3",
                )
                os.makedirs(os.path.dirname(seg_path), exist_ok=True)
                with open(seg_path, "wb") as f:
                    f.write(b"\xff\xfb\x10\x00")
                duration_by_path[seg_path] = seg_duration
                segment_rows.append({
                    "index": i,
                    "tts_path": seg_path,
        ...
        if speedup_raises is not None:
            ...
            def fake_speedup(segs, voice_id, task_dir, variant=None, **kw):
                ...
                duration_by_path[out] = sum(seg_durations)
                segment_rows = []
                for i, seg_duration in enumerate(seg_durations):
                    seg_path = os.path.join(
                        task_dir, "tts_segments", f"{variant}_speedup",
                        f"seg_{i:04d}.mp3",
                    )
                    os.makedirs(os.path.dirname(seg_path), exist_ok=True)
                    with open(seg_path, "wb") as f:
        ...
        def fake_assemble(selected, task_dir, variant):
            ...
            for i, seg in enumerate(selected):
                seg_copy = dict(seg)
                seg_copy.setdefault("index", seg_copy.get("segment_index", i))
                seg_copy["tts_duration"] = seg_copy.get(
                    "tts_duration", seg_copy.get("duration")
                )
                selected_segments.append(seg_copy)
            duration_by_path[out] = sum(float(s["tts_duration"]) for s in selected_segments)
            called["assembled"].append({
                "variant": variant,
                "duration": duration_by_path[out],
                "segments": selected_segments,
            })
            return {"full_audio_path": out, "segments": selected_segments}
        monkeypatch.setattr("pipeline.tts.assemble_full_audio_from_segments", fake_assemble)
    ...
    def test_speedup_triggered_when_audio_in_window(self, tmp_path, monkeypatch):
        ...
        assert round_rec["speedup_post_duration"] == 59.5
    ...
    def test_speedup_skipped_when_final_audio_not_over_video(self, tmp_path, monkeypatch):
        ...
        called = self._common_patches(monkeypatch, audio_dur=59.5)
        runner = self._make_runner()
        result = self._run(runner, tmp_path, video_duration=60.0)
        round_rec = result["rounds"][0]
    ...
    def test_final_overshoot_speedup_hit_adopts_candidate(self, tmp_path, monkeypatch):
        ...
        called = self._common_patches(
            monkeypatch, audio_dur=61.5, speedup_dur=59.8,
        )
        runner = self._make_runner()
        result = self._run(runner, tmp_path, video_duration=60.0)
        round_rec = result["rounds"][0]
        ...
        assert result["tts_segments"][0]["tts_duration"] == 59.8
        assert round_rec.get("speedup_applied") is True
        assert round_rec["speedup_pre_duration"] == 61.5
        assert round_rec["speedup_post_duration"] == 59.8
        assert round_rec["speedup_hit_final"] is True
        assert round_rec["segment_assembly_hit"] is True
        assert round_rec["final_reason"] == "converged_segment_assembly_refined"
        assert round_rec["speedup_context"] == "stage1_converged_postprocess"
    ...
    def test_final_overshoot_segment_assembly_adopts_video_capped_mix(
        ...
        """收敛音频 > 视频时，最多三路 speed 候选进入段级组装，命中 [v-1, v] 即采用。"""
        called = self._common_patches(
            monkeypatch,
            audio_dur=[61.5, 59.5],
        ...
        assert round_rec["speedup_final_audio_choice"] == "assembly"
        assert round_rec["final_reason"] == "converged_segment_assembly_refined"
        selected_without_voice_settings = [
            {k: v for k, v in s.items() if k != "voice_settings"}
            for s in round_rec["segment_assembly_selected"]
        ]
        assert selected_without_voice_settings == [
            {
                "round": 1,
                "segment_index": 0,
                "source": "speedup",
    ...
    def test_final_overshoot_segment_assembly_miss_truncates_shorter_closest_over(
        self, tmp_path, monkeypatch,
    ):
        called = self._common_patches(
            monkeypatch,
        ...
        assert round_rec["segment_assembly_candidate_count"] == 64
        assert round_rec["segment_assembly_min_duration"] == pytest.approx(60.5)
        assert round_rec["segment_assembly_closest_over_duration"] == pytest.approx(60.5)
        assert round_rec["segment_assembly_fallback_applied"] is True
        assert round_rec["segment_assembly_truncated"] is True
        assert round_rec["segment_assembly_pre_truncation_duration"] == pytest.approx(60.5)
        assert round_rec["segment_assembly_post_truncation_duration"] == pytest.approx(60.0)
        assert round_rec["segment_assembly_removed_duration"] == pytest.approx(0.5)
        assert round_rec["segment_assembly_duration"] == pytest.approx(60.0)
        assert round_rec["segment_assembly_untrimmed_audio_path"].endswith(
            "tts_full.round_1.segment_assembly.assembled.mp3"
        )
        assert round_rec["segment_assembly_audio_path"].endswith(
            "tts_full.round_1.segment_assembly.truncated.mp3"
        )
        assert [s["duration"] for s in round_rec["segment_assembly_closest_over_selected"]] == [
            pytest.approx(20.2), pytest.approx(20.2), pytest.approx(20.1),
        ]
        assert [s["duration"] for s in round_rec["segment_assembly_selected"]] == [
            pytest.approx(20.2), pytest.approx(20.2), pytest.approx(20.1),
        ]
        assert [s["tts_duration"] for s in result["tts_segments"]] == [
            pytest.approx(20.2), pytest.approx(20.2), pytest.approx(19.6),
        ]
    ...
    def test_final_overshoot_segment_assembly_miss_keeps_original_when_not_improved(
        self, tmp_path, monkeypatch,
    ):
        called = self._common_patches(
            monkeypatch,
    ...
    def test_stage1_segment_assembly_miss_truncates_closest_over_inside_final_window(
        self, tmp_path, monkeypatch,
    ):
        called = self._common_patches(
            monkeypatch,
        ...
        runner = self._make_runner()
        result = self._run(runner, tmp_path, video_duration=26.841995)
        round_rec = result["rounds"][0]
        ...
        assert round_rec["segment_assembly_closest_over_duration"] == pytest.approx(27.193469)
        assert round_rec["segment_assembly_truncated"] is True
        assert round_rec["segment_assembly_pre_truncation_duration"] == pytest.approx(27.193469)
        ...
        assert [s["tts_duration"] for s in result["tts_segments"]] == [
            ...
            pytest.approx(2.638367),
            pytest.approx(9.795918),
            pytest.approx(5.250612),
            pytest.approx(3.593),
        ]
        assert round_rec["speedup_final_audio_choice"] == "assembly_truncated"
        assert round_rec["final_reason"] == "converged_segment_assembly_truncated"
    ...
    def test_stage1_short_audio_uses_slow_speed_feedback_to_lengthen(
        self, tmp_path, monkeypatch,
    ):
        called = self._common_patches(
            monkeypatch,
        ...
        assert result["final_round"] == 1
        assert called["speedup_speeds"] == [0.97, 0.96]
        assert round_rec["segment_assembly_hit"] is True
    ...
    def test_best_pick_overrun_truncates_closest_over_segment_assembly(
        self, tmp_path, monkeypatch,
    ):
        """Best-pick should crop the best assembled fallback, not the original long round."""
        called = self._common_patches(
            monkeypatch,
            audio_dur=[33.5, 42.4, 28.0, 27.0, 26.0],
            audio_segments=[14.2, 14.1, 14.1],
        ...
        assert [s["tts_duration"] for s in result["tts_segments"]] == [
            pytest.approx(13.0),
            pytest.approx(13.0),
            pytest.approx(11.872),
        ]
        assert round_rec["speedup_context"] == "best_pick_overrun"
        assert round_rec["segment_assembly_hit"] is False
        assert round_rec["segment_assembly_fallback_applied"] is True
        assert round_rec["segment_assembly_truncated"] is True
        assert round_rec["segment_assembly_closest_over_duration"] == pytest.approx(39.0)
        assert round_rec["segment_assembly_pre_truncation_duration"] == pytest.approx(39.0)
        assert round_rec["segment_assembly_post_truncation_duration"] == pytest.approx(37.872)
        assert round_rec["segment_assembly_duration"] == pytest.approx(37.872)
        assert round_rec["speedup_final_audio_choice"] == "assembly_truncated"
        assert round_rec["final_reason"] == "best_pick_segment_assembly_truncated"
        assert round_rec.get("hard_overrun_guard_applied") is not True
