...
def test_db_dump_key_uses_previous_day_under_db_prefix(monkeypatch):
    ...
    key = tos_backup_job.build_db_dump_key(
        backup_date=tos_backup_job.previous_backup_date(datetime(2026, 4, 28, 1, 0, 0)),
        run_time=datetime(2026, 4, 28, 1, 0, 0),
    )

    assert key == "DB/test/2026-04-27/appdb_2026-04-27_010000.sql.gz"
...
def test_upload_mysql_dump_uploads_dump_to_db_key(monkeypatch, tmp_path):
    ...
    result = tos_backup_job.upload_mysql_dump(run_time=datetime(2026, 4, 28, 1, 0, 0))

    assert uploaded == [(dump_path, "DB/test/2026-04-27/appdb_2026-04-27_010000.sql.gz")]
    assert result["backup_date"] == "2026-04-27"
    assert result["object_key"] == "DB/test/2026-04-27/appdb_2026-04-27_010000.sql.gz"
