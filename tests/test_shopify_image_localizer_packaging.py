...
def test_pyinstaller_spec_excludes_playwright_report_assets() -> None:
    spec_path = (
        Path(__file__).resolve().parents[1]
        / "tools"
        / "shopify_image_localizer"
    ...
    assert "/driver/package/lib/vite/traceViewer/" in spec
    assert "/driver/package/lib/vite/htmlReport/" in spec
    assert "_is_optional_playwright_data" in spec
