...
def test_build_inventory_rows_classifies_meta_hot_post_video_covers(monkeypatch, tmp_path):
    from appcore import tos_file_management as mgr
    from appcore.tos_backup_references import ProtectedFileRef
    from appcore.tos_channel_migration import TosChannelConfig

    cover = tmp_path / "output" / "meta_hot_posts" / "video_covers" / "20" / "thumbnail.jpg"
    ...
    monkeypatch.setattr(
        mgr.tos_backup_references,
        "collect_protected_file_refs",
        lambda: [ProtectedFileRef(str(cover), ("meta_hot_post_video_cover",), ())],
    )
    monkeypatch.setattr(
        mgr.tos_backup_storage,
        "backup_object_key_for_local_path",
        lambda local_path: "FILES/prod/opt/autovideosrt/output/meta_hot_posts/video_covers/20/thumbnail.jpg",
    )
    monkeypatch.setattr(mgr, "_head_target_object", lambda *args, **kwargs: {"exists": True, "size_bytes": 5})
    ...
    assert row.module_code == "meta_hot_posts"
    assert row.file_type == "cover"
    assert row.source_labels == ("meta_hot_post_video_cover",)
    assert row.sync_status == "synced"
