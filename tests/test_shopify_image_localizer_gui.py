...
def test_main_ensures_google_first_cdp_browser_before_gui(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(app_main, "_has_other_shopify_localizer_instances", lambda: False)
...
def test_main_legacy_instance_exit_releases_guard_and_does_not_start(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from tools.shopify_image_localizer import main as app_main
    ...
    monkeypatch.setattr(
        ...
        "single_instance",
        SimpleNamespace(
            acquire_or_prompt=lambda close_existing: calls.append(("acquire_guard",)) or guard,
            confirm_close_previous_instance=lambda: calls.append(("confirm",)) or False,
        ),
        raising=False,
    )
    monkeypatch.setattr(app_main, "_has_other_shopify_localizer_instances", lambda: True)
    monkeypatch.setattr(
        app_main,
        "_kill_other_shopify_localizer_instances",
        lambda: calls.append(("kill_existing",)),
...
def test_main_legacy_instance_can_close_previous_then_start(monkeypatch: pytest.MonkeyPatch) -> None:
    from tools.shopify_image_localizer import main as app_main
    ...
    monkeypatch.setattr(
        ...
        SimpleNamespace(
            acquire_or_prompt=lambda close_existing: calls.append(("acquire_guard",)) or guard,
            confirm_close_previous_instance=lambda: calls.append(("confirm",)) or True,
        ),
        raising=False,
    )
    monkeypatch.setattr(app_main, "_has_other_shopify_localizer_instances", lambda: True)
