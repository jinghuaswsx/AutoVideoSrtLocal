...
cp .env.example .env             # 填入凭据；FLASK_SECRET_KEY 必填
python db/migrate.py             # 执行 schema（连测试环境 MySQL）
python db/create_admin.py        # 初始化管理员
python main.py                   # 默认 http://127.0.0.1:5000
```

必填凭据（见 `config.py` 的 `REQUIRED_CREDENTIALS`）：`VOLC_API_KEY` / `TOS_ACCESS_KEY` / `TOS_SECRET_KEY` / `OPENROUTER_API_KEY` / `ELEVENLABS_API_KEY` / `FLASK_SECRET_KEY`。Gemini key 可写在 `google_api_key` 文件（带 `AISTUDIO:` / `CLOUD:` 标签）或环境变量。
...
| OpenAPI | `web/routes/openapi_materials.py`（`OPENAPI_MEDIA_API_KEY` 鉴权） |
