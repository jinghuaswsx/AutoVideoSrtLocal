...
- `登录shopify店铺` 按钮**只**打开 Shopify 主入口 `https://admin.shopify.com/`（不带 `/store/<slug>/`），由用户登录后人工选择目标店铺。**Chrome 必须用普通模式 `session.start_chrome` 启动（无 `--remote-debugging-port`）**——`admin.shopify.com` 主入口前置 Cloudflare 反 bot 检测，CDP 标志会让人机验证码长时间不出现，用户根本登不进去（已知事故 2026-05-09）。
- slug 抓取走「**已登录**」按钮（`confirm_shopify_login_capture_slug`）：用户登录到目标店铺主页后**手动**点这个按钮，程序读 `Default/History` SQLite（`mode=ro&immutable=1`，Chrome 在跑也能读）取 `last_visit_time DESC LIMIT 1` 的 admin store URL，提取 slug 写入 `shopify_image_localizer_config.json` 的 `shopify_domain_store_slugs`。**不要**回到 daemon thread 自动监听——thread 启动时机太早会抓到旧 URL（如硬编码 slug 时代留下的 `/store/omurio/products`），导致 cache 写错（已知事故 2026-05-09）。
- 后续 `settings.shopify_store_slug_for_domain(domain)` **优先**用缓存，缺失再回退内置 dict / 默认 slug。**不要**在登录按钮里加 CDP 端口、硬编码 store slug、或直接跳应用页（EZ / TAA），会绕过 slug 捕获或破坏 Cloudflare 验证。
...
- 旧户历史同步入口：自动定时任务只跑 `enabled=true` 账户；如需补抓旧户历史日数据，用 `python tools/meta_daily_final_sync.py --date YYYY-MM-DD --mode run --account-code newjoyloo_old` 显式指定，代码会从全部账户中匹配 disabled 旧户。
...
- 同步节奏：`autovideosrt-roi-realtime-sync.timer` 与 `autovideosrt-order-profit-incremental.timer` 都使用 `OnCalendar=*-*-* *:00,20,40:00`，固定在每小时 :00 / :20 / :40 触发，两条 cron 同时点燃。历史背景：早期 ROI 是 `OnCalendar=*:02:00`（每小时），订单同步是 `OnUnitActiveSec=20min` drift 模式；2026-05-09 起改为对齐 OnCalendar 后两边永不漂移，便于人工对账。timer 文件源在 [`deploy/server_browser/autovideosrt-roi-realtime-sync.timer`](deploy/server_browser/autovideosrt-roi-realtime-sync.timer) / [`deploy/server_browser/autovideosrt-order-profit-incremental.timer`](deploy/server_browser/autovideosrt-order-profit-incremental.timer)，改动后必须 `sudo cp` 到 `/etc/systemd/system/` 并 `daemon-reload + restart` 两个 timer，部署 SOP 不会自动同步这两个 unit。
...
- 所有会连接 `META_AD_EXPORT_CDP_URL` / `DXM01-Meta` 的 Meta Ads Manager 操作必须走同一把 OS 级文件锁，默认锁文件为 `/data/autovideosrt/browser/runtime-meta-ads/automation.lock`。锁只能用 `fcntl.flock` / Windows 文件锁这类随进程退出自动释放的机制，不允许用“创建文件后删除文件”判断占用；等待锁必须有超时，不能无限挂起。Web 手动同步、ROI 实时同步、收盘日同步、临时旧户回填和直接运行 `scripts/run_meta_ads_backfill_range.py` 都必须遵守这条规则。
...
- 实时大盘「订单盈亏明细」/「已分摊广告费」（spec: docs/superpowers/specs/2026-05-08-analytics-business-date-alignment-fix.md 第 11/12 条 + 2026-05-09 AUT-29 实施补丁）：日终广告表未到、`order_profit_lines.ad_cost_usd=0` 时，[`appcore/order_analytics/realtime.py::_get_realtime_order_profit_details(_for_range)`](appcore/order_analytics/realtime.py) **必须**调一遍 `_apply_realtime_ad_cost_adjustments`（共用 [`order_profit_aggregation::_load_realtime_ad_cost_adjustments`](appcore/order_analytics/order_profit_aggregation.py)）按 package id 加 delta，把每行 `ad_cost_usd` 抬到实时分摊值，并同步下调 `order_profit_usd` / `order_profit_with_estimate_usd`。`/order-analytics → 实时大盘 → 订单盈亏明细` 与 `order_profit_summary.ad_cost_usd` 不允许在「Meta 实时表已有数据但日终未生成」的窗口里恒为 0；这与 [`order_profit_aggregation.get_order_profit_list`](appcore/order_analytics/order_profit_aggregation.py) 的兜底口径完全对齐。
- **每个广告账户必须维护独立的 `column_preset`（spec: docs/superpowers/specs/2026-05-09-ads-purchase-value-order-fallback-design.md）**：Meta Ads Manager URL 的 `column_preset=<id>` 是**账户级**绑定，旧户 2110407576446225 那个 ID `1658418688523178` 在 Omurio / newjoyloo_bak 等其它账户下不存在，导致这些账户导出的 CSV 缺 `购物转化价值 / 广告花费回报 (ROAS) - 购物 / 加入购物车次数 / 结账发起次数 / 平均购物转化价值 / 视频平均播放时长 / CPM / 单次链接点击费用` 等列。新增账户时必须 (1) 在该账户的 Meta Ads Manager UI 内手工创建一份含完整列的 column preset；(2) 复制 preset ID 填到 `system_settings.meta_ad_accounts[*].column_preset` 字段；(3) 等下次自动同步生效。`MetaAdAccount.column_preset` 缺省值是旧户 preset，老配置零迁移可继续用。
- **广告分析「购买金额」按订单口径兜底（同上 spec）**：`appcore/order_analytics/meta_ads.fill_purchase_value_from_orders` 在 `get_ads_level_list / get_ads_level_detail` 出口处对**整组** `(ad_account_id, matched_product_code)` 满足 `SUM(purchase_value_usd) == 0 AND SUM(spend_usd) > 0` 的行套用 `dianxiaomi_order_lines` 站内同产品营收，按各行 spend 占组总 spend 比例分摊。命中兜底的行 `purchase_value_source = "order_fallback"`，否则 `"meta"`；API 顶层 `data_quality.status` 在有兜底命中时为 `"fallback_used"`，前端按 `data_quality.purchase_value.note` 提示。决策面只在**整组 0** 时才触发，避免污染老户里"个别广告真零转化"的合理 0。`column_preset` 修复后 Meta CSV 重新带上购买列，下一次同步会让 group_purchase 变正，兜底自动让位 → 没有清理动作。
- **`sync_mode` 通道开关（spec: docs/superpowers/specs/2026-05-09-meta-ads-xhr-token-channel.md，2026-05-09 起）**：每个 account 多了一个 `sync_mode` 字段，取值 `csv_export`（默认，等同旧行为）或 `xhr_api`（in-page Marketing API 通道，绕过 CSV 导出弹窗）。CSV 导出通道**完全保留**，UI 一键切回。`xhr_api` 通道实现：[`appcore/meta_ads_xhr_token.py`](appcore/meta_ads_xhr_token.py) 从 Ads Manager 页面 session 拦 am_tabular 请求抓 `access_token` → 缓存到 `system_settings.meta_xhr_token_cache`（90min TTL）→ [`appcore/meta_ads_in_page_fetch.py`](appcore/meta_ads_in_page_fetch.py) 在 Playwright 页面里 `page.evaluate(fetch ...)` 调 `https://adsmanager-graph.facebook.com/v22.0/act_<id>/insights`（**必须**这个子域 + `credentials:'include'`，外部 urllib + `graph.facebook.com` 都被 Meta 拒绝 OAuth code 1 因为 token 是 first-party session-bound）。一轮 sync 共开一次浏览器、一把锁，所有 xhr_api 账户共享 session。`tools/roi_hourly_sync.py` 与 `tools/meta_daily_final_sync.py` 都按 `account.sync_mode` 分支，xhr 与 csv 账户互不阻塞。
...
- **`timezone` 字段（spec: docs/superpowers/specs/2026-05-09-meta-ads-account-timezone-and-async-fix.md，2026-05-09 起）**：每个 account 多了一个 `timezone` 字段，取 IANA 时区名（默认 `America/Los_Angeles`，覆盖 newjoyloo / newjoyloo_bak / Omurio 三户的 Meta 默认配置）。`xhr_api` 通道**必须**走 [`appcore/meta_ad_accounts.account_xhr_time_range(account, business_date)`](appcore/meta_ad_accounts.py) 把 BJ 业务窗口 `[BJ 16:00 D, BJ 16:00 D+1)` 映射到账户时区下的 calendar date 区间（PDT 通常返回 `{since=D, until=D+1}`、PST 返回 `{since=D, until=D}`、Asia/Shanghai 返回 `{since=D, until=D+1}`），realtime + daily_final 两条 path 共用同一份 helper；**禁止**再写 `time_range={"since": business_date.isoformat(), "until": business_date.isoformat()}` 这种不带账户时区的硬编码（已知事故：2026-05-09 newjoyloo_bak xhr 通道连续多 tick `rows_imported=0`）。CSV 通道走 Ads Manager 渲染、隐式带账户时区，无需改动。UI 入口在「广告账户」Tab 时区列（`<input list="metaAdAccountTimezoneOptions">` + datalist 候选 `America/Los_Angeles` / `America/New_York` / `Asia/Shanghai` / `Europe/London` / `UTC`，可手输任意 IANA 字符串，由 `_coerce_timezone` 在保存时统一校验，AUT-28 落地）。
- **Playwright sync API 与 asyncio loop 隔离（同上 spec）**：[`appcore/meta_ads_in_page_fetch.py::open_meta_ads_session`](appcore/meta_ads_in_page_fetch.py) **必须**先探测当前线程是否有 running asyncio loop（`asyncio.events._get_running_loop()`）；若有，session 内整套 `sync_playwright()` / `connect_over_cdp` / `page.evaluate(...)` 操作都丢到一个 `ThreadPoolExecutor(max_workers=1)` 工人线程上跑，避免触发 Playwright 的 "It looks like you are using Playwright Sync API inside the asyncio loop" 断言。已知事故：2026-05-09 15:18 一次 xhr 通道 tick 因为上游某线程残留 asyncio loop 直接 session 全员失败。新接 session 调用方不要在 `with open_meta_ads_session()` 外层另起 asyncio loop，调用方代码不需要任何改动。
- 改这条链路至少运行 `pytest tests/test_roi_hourly_sync.py tests/test_roi_hourly_sync_meta_multi_account.py tests/test_meta_server_sync_tools.py tests/test_meta_ads_xhr_token.py tests/test_meta_ads_in_page_fetch.py tests/test_meta_ads_account_timezone.py tests/test_order_analytics_ads.py tests/test_product_profit_report.py tests/test_order_profit_aggregation.py tests/test_order_analytics_data_quality.py -q`。
- **`output/meta_daily_final_exports/` owner / mode 约束（2026-05-09 起，spec: docs/superpowers/specs/2026-05-09-meta-daily-final-permission-recovery.md）**：systemd unit `autovideosrt-meta-daily-final-sync.service` / `-check.service` 的 `ExecStartPre=install -d -o root -g root -m 02775` 负责把 export 根目录守在 `02775`（含 setgid，子目录继承组写权限）。**红线**：不要在 prod 上手工 `sudo python tools/meta_daily_final_sync.py ...`（包括 `--account-code newjoyloo_old` 这种"小补一下"的回填），手工 root 跑出来的 `<date>/<ts>/` 子目录会让后续 service 进程 mkdir 失败 → daily_final 在凌晨 PermissionError（已发生：2026-05-07 / 2026-05-09 BJ 00:11 事故）。`tools/meta_daily_final_sync._run_meta_ads_export` 的 `_ensure_export_dir` 会把冲突父目录改名为 `<原名>.conflicted-<bj_ts>` 自动恢复一次并把记录写进 `summary_json.export_dir_recovery`，但运维侧不要主动制造冲突。补抓历史用「广告账户」Tab 的手动同步弹窗（走 Web 进程，跟 service 同 user）；只有走不通时才回到 CLI，且必须 `sudo -u root` 跑（保持 owner 一致）。
- 告警链路：失败告警的去重 / 节流由 [docs/superpowers/specs/2026-05-09-roi-hourly-sync-lock-recovery.md](docs/superpowers/specs/2026-05-09-roi-hourly-sync-lock-recovery.md) 中的 `appcore.feishu_alerts.should_dispatch_failure`（首次失败 + 之后每 N 次，N 默认 5）+ `_dispatch_recovery_alert` 共同负责；`meta_daily_final` 失败被该链路覆盖。`_dispatch_failure_alert` 在调度发送时若 streak ≥ 2 会把 `consecutive_failures` 注入 row，飞书消息体多一行 `连续失败：N 次`，方便事故定位。
