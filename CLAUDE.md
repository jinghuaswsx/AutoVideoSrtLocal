...
- `登录shopify店铺` 按钮**只**打开 Shopify 主入口 `https://admin.shopify.com/`（不带 `/store/<slug>/`），由用户登录后人工选择目标店铺。**Chrome 必须用普通模式 `session.start_chrome` 启动（无 `--remote-debugging-port`）**——`admin.shopify.com` 主入口前置 Cloudflare 反 bot 检测，CDP 标志会让人机验证码长时间不出现，用户根本登不进去（已知事故 2026-05-09）。slug 抓取用 daemon thread 周期性 `sqlite3.connect(file:.../Default/History?mode=ro&immutable=1)` 读 Chrome History 表里最新的 `https://admin.shopify.com/store/<slug>/...` URL，写入 `shopify_image_localizer_config.json` 的 `shopify_domain_store_slugs`。后续 `settings.shopify_store_slug_for_domain(domain)` **优先**用缓存，缺失再回退内置 dict / 默认 slug。**不要**在登录按钮里加 CDP 端口、硬编码 store slug、或直接跳应用页（EZ / TAA），会绕过 slug 自动捕获或破坏 Cloudflare 验证。
