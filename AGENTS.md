...
- 全能翻译质量提升系列（5 块分批实施，音画对齐红线必读）：总览 [specs/2026-06-12-omni-quality-overview.md](docs/superpowers/specs/2026-06-12-omni-quality-overview.md)，含各块 spec + plan 链接
...
- **CC-X7 公网机（隧道）发布**：口令「提交代码，合并代码到master，发布线上生产环境」→ `bash deploy/publish_ccx7.sh prod --confirm -m "<msg>"`（自动 commit + fetch/rebase 安全合并 master + test→prod 不停）；只发测试用 `test`。见 [spec](docs/superpowers/specs/2026-06-14-ccx7-tunnel-deploy-design.md)
## Don't
