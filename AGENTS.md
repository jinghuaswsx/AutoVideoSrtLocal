...
- Windows 开发机仍禁止安装、初始化、启动或依赖本机 MySQL；数据库以服务器 MySQL 为准。
- 当前本机是服务器开发/部署机时，允许在本机运行自动化测试、手工测试、路由测试和数据库相关测试；这些测试必须通过测试环境配置连接测试库/测试服务，不再视为“Windows 本地 MySQL”。
- 禁止误连未配置的默认本机库（例如无密码 `root@127.0.0.1:3306/auto_video`）。如果环境变量或 `.env` 没有明确指向测试环境数据库，应先补测试环境配置，不要用默认本地库兜底。
- 不要在 Windows 开发机本地长期启动项目 Web 服务来替代测试环境；服务器本机可按测试环境配置运行短期 dev/test 命令做验证。
