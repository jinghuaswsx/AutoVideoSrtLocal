...
GPU 加速音频人声/伴奏分离服务，运行在内网 `172.16.254.106:80`。
...
```bash
# 健康检查
curl http://172.16.254.106/health

# 最简调用：上传文件分离（默认使用 vocal_balanced 集成预设）
curl -X POST http://172.16.254.106/separate \
  -F "file=@song.mp3"

# 下载分离结果 ZIP
curl -X POST http://172.16.254.106/separate/download \
  -F "file=@song.mp3" \
  -o separated.zip
```
...
```bash
curl -X POST http://172.16.254.106/prewarm \
  -F "ensemble_preset=instrumental_clean"
```
