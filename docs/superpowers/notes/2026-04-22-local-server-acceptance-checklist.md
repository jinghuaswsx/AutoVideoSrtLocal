...
目标结论：确认 `AutoVideoSrtLocal` 已具备在 `172.16.254.106` 上作为唯一正式生产入口运行的条件；运行时以 `systemd + 本地 MySQL + 本地文件存储` 为主，只有必须公网回拉的链路继续使用 TOS；旧远程服务器保持不动，仅保留为参考和应急回退入口。
...
- 正式验收一律以 `http://172.16.254.106/` 为入口，不使用旧远程地址，不通过临时端口，不通过反向代理假设。
...
在 `172.16.254.106` 上确认正式服务由 `systemd` 托管：
...
- 有进程监听 `:80`
- `http://127.0.0.1/` 和 `http://172.16.254.106/` 返回 `200` 或登录态下可接受的 `302`
- 不需要手工带历史端口

补充检查：

- 浏览器从同网段 / VPN 网络直接访问 `http://172.16.254.106/`
...
- 本次发布、重启、验收命令只在 `172.16.254.106` 上执行
...
本块目标：在 `172.16.254.106` 上完成至少一条主翻译任务的真实闭环，验证从上传到下载的完整链路。
...
- 默认上游地址是否指向 `http://172.16.254.106`
- 基础配置读取正常
- 与本地服务通信正常
...
- 旁路服务处于可接受状态
- 团队确认正式入口统一为 `http://172.16.254.106/`

### 6.2 回退口径

若本地服务器出现阻断性故障：

1. 立即停止继续把正式流量导向 `172.16.254.106`
2. 恢复团队使用旧远程入口
3. 保留本地机器现场，不做破坏性清理
