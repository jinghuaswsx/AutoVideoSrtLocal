...
```bash
ssh -i "C:\Users\admin\.ssh\CC.pem" root@172.16.254.106 "cd /opt/autovideosrt && mysql -u root -p'***' auto_video < db/migrations/002_user_voices_and_prompts.sql"
```
...
```bash
ssh -i "C:\Users\admin\.ssh\CC.pem" root@172.16.254.106 "cd /opt/autovideosrt && git pull origin master && mysql -u root -p'***' auto_video < db/migrations/002_user_voices_and_prompts.sql && systemctl restart autovideosrt"
```
