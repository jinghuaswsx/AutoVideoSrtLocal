...
Add tests covering `record_push_material_approved()` and `record_push_material_skipped()` so they prove child status becomes `done`, completion events are written, and parent status can become `all_done`. Add a regression test proving `cancelled` child tasks are not completed by skip-push.
...
Add tests proving `/mark-pushed` and `/skip` call the corresponding task service when historical target-language material has no `task_id` and must be resolved by product/language. Add a route test proving an English/source material skip does not complete child tasks bound through `tasks.media_item_id`.
...
Add tests proving active child tasks are completed when a matching target-language material is already `skip_push=1`, including unbound historical rows matched by product/language.
