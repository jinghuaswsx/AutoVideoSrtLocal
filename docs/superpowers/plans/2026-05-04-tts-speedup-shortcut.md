...
## Task 2: 注册 LLM use_case `tts.speedup_quality_review`
...
```
Expected: FAIL — `KeyError: 'unknown use_case: tts.speedup_quality_review'` 或类似。
...
git -C .worktrees/tts-speedup-shortcut commit -m "$(cat <<'EOF'
feat(llm): 注册 tts.speedup_quality_review use_case
...
USE_CASE_CODE = "tts.speedup_quality_review"
...
- 每次变速 pass 都会**同步**调用 `tts.speedup_quality_review`（默认 OpenRouter + google/gemini-3-flash-preview）做双轨对比 AI 评分，120s 超时不阻塞任务。结果写入 `tts_speedup_evaluations` 表。
