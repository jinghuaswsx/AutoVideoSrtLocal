  ...
  "issues": [
    {
      ...
      "evidence": "简短证据",
      "sync_point": "同步点，例如 ASR 3（00:10.20-00:12.00）",
      "sentence_text": "导致风险的目标语句或 TTS 文本",
      "timing_detail": "目标画面时长、音频时长、超出/不足多少秒",
      "recommendation": "中文处理建议：音频变速 / 重写文案后重新生成音频 / 重新生成音频 / 人工复核",
...
```json
{
  "av_sync_audit": {
    ...
    "audit_timeline": [
      {
        "asr_index": 3,
        "asr_text": "source ASR text",
        "target_text": "final target/TTS text",
        "visual_observation": "这一段对应的实际画面内容",
        "diagnosis_status": "ok | issue",
        "diagnosis": "逐段诊断意见",
        "verified": true
      }
    ],
...
`audit_timeline` 是前端音画同步审计的主展示数据，必须按 ASR 时间顺序展开。每一行对应一段 ASR，包含 ASR 原文、最终目标语/TTS 文案、实际画面内容、诊断意见和复核状态；没有问题的段落也要输出，方便人工按时间顺序审片。Omni `run()` 在 Gemini verify 后可将已确认问题标记为 `verified=true`；多语种 `run_report_only()` 只做辅助分析，不进入 verify，因此同样输出时间线但不标记复核通过。
...
- 审计结论必须用中文面向运营人员表述，不能只展示原始 JSON。
- 结论必须明确列出问题同步点、问题句子、目标画面时长、TTS 音频时长、偏差秒数，以及“音频太长/太短导致画面对不上”等原因。
- 每个确认问题必须给出处理建议，建议范围限定为：小幅音频变速、重写/压缩或扩写文案后重新生成音频、仅重新生成音频、人工复核。
...
- 修正后时长变差会回滚。
- 审计 artifact 会生成中文 `human_report`，并对诊断/复核问题补齐 `sync_point`、`sentence_text`、`timing_detail`、`recommendation`。
