...
6. 新品范围默认 7 天，可在页面选择 3 / 7 / 15 / 30 / 60 天。
7. 新品：`ad_launch_date >= 今天00:00 - product_launch_window_days`。
8. 老品：`ad_launch_date < 今天00:00 - product_launch_window_days`。
9. 新老品归类按当前今天 00:00 固定判断，不跟随页面选择的历史日期范围变化。
10. 历史产品允许一次性回填上广告时间。
11. 找不到真实广告匹配记录时，先用 `media_products.created_at` 的日期作为临时上广告时间。
12. 后续第一次真实匹配到广告数据时，需要把 fallback 上广告时间更新为真实广告匹配日期。
13. 一旦来源变为真实广告匹配，后续不再因更晚广告数据改写，除非未来有明确重算需求。
14. 新品投放分析下设三个子 Tab：
    - 新品分析
    - 老品数据
    - 未匹配产品
...
| `product_launch_window_days` | `3` / `7` / `15` / `30` / `60` | 新品窗口天数；默认 7；仅影响 `new` / `old` 的产品集合 |
...
- 实时大盘顶部筛选区新增“新品范围”下拉框，选项为 3 / 7 / 15 / 30 / 60 天，默认 7 天；下拉框放在店铺筛选旁边。
- 新品投放分析筛选区也显示同一个“新品范围”下拉框；实时大盘与新品投放分析共用当前选择，切换一级 Tab 不重置。
- 实时大盘顶部新品 / 老品卡片说明必须直接展示业务口径：新品为“上广告时间近 N 天内”，老品为“上广告时间 N 天前”，并保留范围产品数量，避免只显示 `product_launch_scope` 技术参数。
- 用户选择新品范围后，顶部新品卡片的头部标识、范围产品数量、全部 KPI 数据和说明文案必须同步重算；老品卡片也按同一边界同步重算。
- 增加三段式子 Tab：
  - `新品分析`
  - `老品数据`
  - `未匹配产品`
...
2. 新品/老品按北京时间今天 00:00 与 `ad_launch_date` 差值判断，不随查询历史日期范围变化；窗口天数按页面选择的 3 / 7 / 15 / 30 / 60 天计算。
...
11. 实时大盘选择新品范围后，新品/老品卡片的头部标识、范围产品数量、KPI 数据和说明文案同步变化。
12. 新品投放分析与实时大盘使用同一新品范围选择，接口请求携带 `product_launch_window_days`。
...
- 扩展实时大盘测试：
  ...
  - `product_launch_window_days=3/7/15/30/60` 使用对应 cutoff；非法值返回 400。

前端：

- 模板静态测试覆盖一级 Tab `新品投放分析` 与三个子 Tab。
- JS 请求参数包含 `product_launch_scope`。
- 实时大盘与新品投放分析的“新品范围”下拉框选项、默认值和请求参数覆盖。
- 卡片文案随 `product_launch_window_days` 动态显示 “近 N 天内 / N 天前”。
