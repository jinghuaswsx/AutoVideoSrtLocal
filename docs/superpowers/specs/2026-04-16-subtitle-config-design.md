...
## 8. 2026-05-08 所见即所得坐标契约修订

`subtitle_position_y` 仍表示“距顶百分比”，但用于预览和生成时必须统一为**字幕渲染外框底边**的位置，而不是字幕块中心点。

原因：硬字幕生成端使用 ffmpeg subtitles/libass 的 `Alignment=2` 和 `MarginV`。在该渲染模型下，`MarginV=PlayResY*(1-position_y)` 约束的是底部对齐字幕的下边界/基线区域；实测 `position_y=0.68` 时，270×480 画面中白字外框底边落在约 326px，等于 480×0.68。若前端预览用 `top: 68%` + `translateY(-50%)` 把 68% 当作字幕块中心，预览会比实际生成视频低约半个字幕块高度，用户看到的位置和最终硬字幕不一致。

硬规则：

- 所有字幕预览组件（任务详情页选音色阶段、共享字幕预览组件、素材翻译弹窗）必须把 `subtitle_position_y` 映射为字幕外框底边：CSS 使用 `top: calc(position_y*100%)` 且纵向锚点为 `translateY(-100%)`。
- 会改变字幕块高度但不会出现在生成视频里的提示文案、操作说明、辅助 label，不得放入同一个被定位的字幕块内部；否则会参与高度计算，破坏所见即所得。
- 拖拽或滑块更新时，写回的仍是 `subtitle_position_y` 原值，不新增第二套坐标字段。
