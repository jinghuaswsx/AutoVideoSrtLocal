...
### 3.4 ElevenLabs 瞬时冲突退避重试

[pipeline/tts.py](../../../pipeline/tts.py) 现有 `_call_with_network_retry` 只识别网络层异常（`httpx.RemoteProtocolError` 等），不识别 ElevenLabs HTTP 层的瞬时失败。新增 `_call_with_throttle_retry` 包一层，覆盖两类可短暂重试的返回：

- HTTP `429`：`concurrent_limit_exceeded` / `rate_limit_exceeded`，表示超出套餐或账号并发额度。
- HTTP `409`：`already_running` 且 body 包含 `Multiple voice additions/deletions for the same voice were called at the same time`。线上在 2026-05-08 已观察到 text-to-speech 并发请求同一 shared/library voice 时会返回这个冲突；服务端提示 `Please retry shortly`，应与 429 一样按短退避重试，而不是直接让整轮 TTS 失败。
...
def _is_concurrent_limit_429(exc: BaseException) -> bool:
    """识别 ElevenLabs 可短暂重试的 TTS 并发/冲突错误。"""
    ...
    for attempt, delay in enumerate(_THROTTLE_RETRY_DELAYS):
        try:
            ...
            if attempt >= len(_THROTTLE_RETRY_DELAYS) - 1:
                ...
                raise
            log.warning("%s retryable throttle/conflict, retry in %.1fs: %s", label, delay, exc)
            time.sleep(delay)
```

`generate_segment_audio` 内部调用顺序：throttle_retry → network_retry → SDK convert（throttle 在外层，因为 429/409 是 HTTP 层错误，已经收到 response）。
...
| 单段 HTTP 429 / concurrent_limit_exceeded | `_call_with_throttle_retry` 兜底，0.5/1/2/4s 退避，4 次后抛 |
| 单段 HTTP 409 / already_running / voice additions-deletions | `_call_with_throttle_retry` 同样兜底，按 ElevenLabs "retry shortly" 语义短退避重试；非该 code/message 的 409 透传 |
...
5. 任一段返回 429 或 `409 already_running` 时，单元测试覆盖：先退避、退避耗尽抛错、非可重试 409 透传、上层 duration loop 进入下一轮 rewrite。
