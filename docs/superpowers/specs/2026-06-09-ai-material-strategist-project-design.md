...
## 2026-06-11 AI素材军师 Top 30 补素材建议

用户要求 `AI素材军师` 从原 Top 20 改为 Top 30，确保一次项目输出 30 个产品的补素材建议。

- `appcore.ai_material_strategist` 的最终项目产品数为 30；当排名模型返回不足 30 个产品时，必须继续按规则候选顺序补齐到 30 个，除非候选总量本身少于 30。
- 排名流程继续先从规则预筛候选中取前 60 个进入 AI 复评；候选按 20 个一批复评，三批各保留最多 10 个进入决赛，决赛提示词必须明确要求输出最终 Top 30。
- 逐产品分析、断点续跑、进度 `product_progress.total`、旧结果清理和项目汇总都必须跟随本轮实际选中的 30 个产品，不得仍按 20 个产品截断。
- 页面文案、项目提示词可视化和项目列表统计必须避免继续写死 Top 20。
- 单品 AI 返回可读结论但漏掉 `priority`、`primary_action`、`next_check` 等结构化字段时，服务端必须用同产品的确定性 fallback 补齐缺失字段，保留 AI 的 `overall_judgement`，避免 30 个结果里出现不可统计的空建议。

## 2026-06-11 AI素材军师报告素材卡片与自制 EN 素材源

用户要求 `AI素材军师` 报告里的视频素材卡片对齐 `明空选品` 的视频素材卡片效果，并把国家反馈放到每个产品详情区最左侧，后面紧跟素材候选卡片。

- 报告素材卡片必须复用 `明空选品` 卡片的信息密度和结构：顶部入库/视频状态图标、产品中文名、英文名、产品 code、素材标题、产品链接、投放热度、90 天消耗、昨日消耗、图片/视频 Tab、广告数、上传者、上传时间和操作按钮区域。
- 顶部状态图标必须复用 `明空选品` 语义：只有产品已入本地素材库时显示 📦 产品图标，只有该视频素材已入本地素材库时显示视频图标；未入库候选只保留布局空位，不能亮起对应状态图标。
- 每次修正报告卡片前端逻辑时必须同步更新 `ai_material_strategist.js` 的模板版本参数，避免浏览器继续使用旧的写死图标脚本。
- 公开分享报告仍不能暴露后台内部链接或可执行后台操作；公开页只保留安全的视频预览与文本状态。后台详情页可以保留加入素材库、素材工作台等操作入口。
...
本功能新增 `投放素材AI分析`，每次运行生成一个独立项目。项目保存当次输入数据、AI 提示词、模型输出、Top 30 产品、逐产品建议和可执行操作入口。项目详情页是完整独立路由，可随时回看当次结论。
...
3. 找出当前产品里综合表现最好的 30 个产品，必须同时考虑“量”和“ROAS”，不能让 1-2 单高 ROAS 产品冒头。
4. 对 Top 30 每个产品单独分析投放、订单、国家、素材翻译反馈、明空素材候选，并给出补素材建议。
...
将候选按 20 个一批分 3 次调用。每批输入压缩指标，输出每批 Top 10 与理由。最后再把 3 批候选合并调用一次总排名，输出最终 Top 30。
...
每个 Top 30 产品单独调用，不合并成一次调用。单产品数据包包含：
...
- 项目记录是断点载体：
  ...
  - `ranking_prompt_json`、`ranking_result_json` 存在时，恢复时复用 Top 30 排名结果，不重复调用排名模型。
  - `ai_material_strategist_product_results` 已存在的产品结果视为已完成，恢复时从下一个未完成产品继续逐产品分析。
- 逐产品分析必须边跑边落库。完成一个产品就写入 `ai_material_strategist_product_results`，并更新 `progress_json.product_progress`；不能等 30 个产品全部跑完才统一写入。
...
- 启动恢复必须二选一收口：能成功排队恢复的项目保持 `status='running'` 并写入 `runner_state='resume_scheduled'`；不能排队恢复的项目必须落为 `status='interrupted'`，前端显示“中断”，不得留下没有执行器的假“运行中”状态。
- 每个步骤卡片都必须提供管理员可见的「从此步继续」按钮。点击后仍复用同一个 `project_id`，服务端按步骤起点清理后续断点并重新排队执行：`snapshot` 清空快照、排名、产品结果和汇总；`candidate_score` 保留快照，清空排名、产品结果和汇总；`ai_ranking` 清空排名、产品结果和汇总；`material_context` / `product_analysis` 保留快照和排名，清空产品结果和汇总；`persist` / `summary` 保留已落库产品结果，只重新整理持久化和汇总。
- 「从此步继续」必须先获取 AI素材军师单任务锁；如果当前执行器仍在运行，接口返回 `409`，避免旧线程继续写入时清理断点。服务重启后的 stale running 项目没有活跃锁时，可以直接被手动按钮接管。

### 2026-06-11 断电续传收口

「继续未完成」和「从此步继续」是两个不同动作：

- 「继续未完成」用于断电、服务重启或后台线程中断后的严格续传。它必须复用同一个 `project_id`、保留 `data_snapshot_json`、`ranking_result_json` 和所有已写入的 `ai_material_strategist_product_results`，重新排队执行器后从第一个未完成产品继续。该动作不得删除产品结果，避免已完成产品重复跑。
- 「从此步继续」用于人工选择步骤起点重跑。它按上文步骤清理后续断点，语义是“从步骤起点重新执行”，不是严格续传。
- 如果项目 `status='running'` 但 `progress_json.runner_state` 为 `resume_scheduled` / `manual_resume_scheduled` 且 `runner_heartbeat_at` 长时间未推进，或 `runner_heartbeat_at` 距当前时间超过 10 分钟，页面必须允许管理员使用「继续未完成」接管。接口仍必须先获取单任务锁；锁忙说明真实执行器仍在运行，返回 `409`。
- Web 启动恢复只自动拉起最新 running 项目；对其他 running 项目必须标记为 `interrupted` 并写入 `recovery.reason='replaced_by_startup_recovery'`，避免多条假 running 阻塞新建项目或误导页面。
- 如果最新 running 项目自动恢复排队失败，服务端必须把该项目标记为 `interrupted`，并同步收口其他 running 项目；不得留下没有执行器的 running 项目。
- 项目详情 API 必须返回轻量恢复状态，例如 `can_resume_checkpoint` / `resume_checkpoint_reason`，供前端判断 running 但可接管的项目是否显示「继续未完成」。
...
- 顶部按钮：`新建项目 / 运行一轮`。
- 项目列表：项目名、状态、生成时间、Top 30 数量、P0/P1 建议数、失败原因。
- 点击项目进入详情页。
...
- 项目详情渲染 Top 30 表、图表容器、国家矩阵、素材卡片和操作按钮。
