...
2. `product_links` **不再只放当前语种**，改为读取 `media_languages` 里所有 `enabled=1` 的语种（包含英语 `en`），按产品链接域名规则拼出完整链接数组
...
```json
{
  ...
  "product_links": [
    "https://newjoyloo.com/products/sonic-lens-refresher-rjc",
...
- payload 组装：`videos` 长度 = 1；`product_links` = 启用语种数 × 链接数（包含英语）
