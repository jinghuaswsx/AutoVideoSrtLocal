...
素材管理的 SKU 配对详情弹窗当前已经能展示 SKU 维度的“估算保本 ROAS”。这个估算值主要来自 SKU 的售价、采购价和小包成本聚合，适合做没有订单样本时的初始参考，但不能反映已经产生订单后的真实成本结构。

运营需要在每个 ERP SKU 上新增一列“订单保本 ROAS”，放在“估算保本 ROAS”后面。这个值基于实际订单数据计算，覆盖售价、采购成本、实际物流费用和手续费成本；没有足够订单数据时显示横杠。
...
1. 在素材管理 SKU 配对详情中，于“估算保本 ROAS”后新增一列“订单保本 ROAS”。
2. 每天北京时间 1:00 计算一次，不在页面打开时实时聚合订单。
3. 每次计算使用滚动 30 个已稳定业务日：运行日为 D 时，统计 `D-32` 到 `D-3`，含首尾。
4. 有窗口内订单数据的 SKU 显示订单保本 ROAS；没有订单数据的 SKU 显示横杠。
5. 手续费来源显示为一个标签：`手续费真实`、`手续费7%估算` 或 `手续费部分真实`。
6. Shopify Payments 数据后到时，允许重算并把手续费来源从 `手续费7%估算` 更新为 `手续费真实` 或 `手续费部分真实`。

## 非目标

- 不改变现有“估算保本 ROAS”的公式和展示位置。
- 不把广告费扣进“订单保本 ROAS”公式。广告费是实际投放表现数据，不属于保本阈值中的成本扣减项。
- 不在素材列表主表新增宽列；本期只改 SKU 配对详情弹窗。
- 不要求 Payment 导入后立即同步重算所有历史窗口；可以先提供可调用重算入口，再由后续实现决定是否在导入接口里触发。

## 核心口径

“订单保本 ROAS”是保本阈值，不是实际投放 ROAS。

```text
订单保本 ROAS = 总销售额 / 可承受广告费
...
| `actual_breakeven_roas` | 订单保本 ROAS；不可保本时为 NULL |
| `computed_at` | 计算时间 |
...
如果两者都有，`fee_source='mixed'`，前端标签显示 `手续费部分真实`。
...
- 主数据口径列名为“估算保本 ROAS”，数字后缀只表达小包成本来源：`小包实费` 或 `小包预估`。
- 订单快照口径列名为“订单保本 ROAS”，数字后缀只表达 Shopify Payments 手续费来源：`手续费真实`、`手续费7%估算` 或 `手续费部分真实`。
- 顶部说明必须区分“主数据估算”和“订单快照”，避免在“估算”“实际”“真实”之间交叉使用含糊短标签。
...
| 全部真实手续费 | `2.34 手续费真实` |
| 全部 7% 估算 | `2.34 手续费7%估算` |
| 部分真实部分估算 | `2.34 手续费部分真实` |
...
- `tests/test_material_roas_frontend.py`
  - SKU 表头包含“估算保本 ROAS”后紧跟“订单保本 ROAS”。
  - 来源标签文本存在。
...
2. **真实手续费覆盖率滞后**：Payment 一周或一个月才导入一次，前端必须明确标出 `手续费7%估算` 或 `手续费部分真实`。
...
1. 素材管理 SKU 配对详情中，“估算保本 ROAS”后出现“订单保本 ROAS”列。
2. 有最近 30 个已稳定业务日订单的 SKU 显示数值和手续费来源标签。
3. 无订单数据或不可保本的 SKU 显示横杠。
4. 每天 1:00 北京时间可由 systemd timer 触发快照计算。
5. Payment 导入后重算窗口时，同 SKU 快照可从 `手续费7%估算` 更新为 `手续费真实` 或 `手续费部分真实`。
