...
推送页面的视频预览拿到的历史 payload URL 仍指向旧服务器 `172.30.254.14`，浏览器无法从该地址拉取视频。当前服务器已切到 `172.16.254.106`，运行代码中也存在多处显式写死当前 IP，后续迁移时容易再次出现同类问题。
...
- 源码扫描测试禁止运行面散落 `172.30.254.14` 或除 `server_config.py` 外的 `172.16.254.106`。
