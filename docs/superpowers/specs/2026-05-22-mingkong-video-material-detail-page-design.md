...
- The cover and video frames share the same centered portrait media viewport, matching
  the video-card preview size from `mk_selection.html` (`216px` by `384px`) so the
  detail page does not stretch media across the full right-side panel.
