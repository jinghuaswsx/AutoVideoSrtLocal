...
- `potential_new_products`：统计所选业务周内 `media_products.created_at` 落在 `week_start` 到 `week_end` 的新品，每次评估都只使用该完整业务周（周日到周六）的 7 天订单、购买结果、日均单量和 ROAS。候选不再强制要求属于周报分级 `测试中`；稳定品、二级稳定品 / 潜力品和停投品不重复进入该卡片。排序仅使用本周日均单量和 ROAS，不读取上广告时间、产品位置或产品属性。
...
- `每周 AI 分析` 增加 `稳定产品分级` 可视化区：
  - 汇总稳定品总数、7 天稳定数、30 天稳定数、潜力品数、测试中数、终止投放数、未投放数；`secondary_stable` 和历史 `potential` 在前端统一显示为 `潜力品`。
  - 明细表只保留一个 `类型` 字段，不再同时展示 `类型` 和 `标记`。稳定品的 `7天稳定` / `30天稳定` 合并进类型文案；未连续 7 天活跃显示 `测试中`；停投显示 `终止投放`。
  - 明细表展示头部稳定品和潜力品的产品、7 天 / 30 天订单、日均、ROAS、投放状态。
...
- `每周 AI 分析` 增加 `潜力新品情况` 可视化区：
  ...
  - 候选使用所选完整业务周（周日到周六）的 7 天订单和购买结果评估，不再要求必须属于周报分级中的 `测试中`；稳定品、二级稳定品 / 潜力品和停投品仍排除，避免重复进入该卡片。
  - 判断表现只看同一周的本周订单、7 天日均单量和 ROAS；不考虑投放时间、产品位置、上架状态、产品属性等额外维度。
  - 默认展示前 10 个，按日均单量降序、ROAS 降序排序。
  - 展示形式与稳定产品分级保持一致，包含产品主图、产品名 / Code 和单一 `类型` 字段；类型显示 `潜力新品 · 测试中`、`潜力新品 · 投放未满7天` 或 `潜力新品 · 周度观察`，不再拆成 `标签` 和 `产品分级` 两列。
...
- 2026-06-08 运行闭环修订：每周 AI 报告只有在整体周报 AI、逐产品推进 AI、JSON 解析和落库全部完成后，才算完整报告。逐产品候选必须实际执行；单产品失败以失败结果入库，不阻断整周报告，但不得再把 `skipped_sync` 当作完成态。
- 2026-06-08 运行闭环修订：页面读取到 `preview`、`failed`、缺少 `ai_report_json`、`product_action_evaluation_summary.mode=skipped_sync` 或候选未评估完成的缓存时，必须标记为不完整，并自动启动该周完整评估任务。完整评估完成后覆盖写入 `weekly_ai_analysis_reports`，之后再次进入页面直接加载该周完整缓存。
...
- 点击 `生成 AI 报告` 不直接沿用页面日期生成，而是打开弹窗选择一周范围；弹窗默认选最近一个完整业务周，选择器只能以周日到周六为单位，任意日期输入都要归一到所在周周日，并展示对应周日到周六范围。
- 页面初次进入或切换周时必须先加载该周数据包和缓存状态；如果缓存不完整且没有任务在跑，前端立即调用后端启动完整评估并进入轮询。轮询期间流程图展示运行中状态，完成后重新加载缓存数据。
