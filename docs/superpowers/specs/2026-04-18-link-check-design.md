...
- 图片整体视觉相似度
  - 可用感知哈希、尺寸归一化后的像素特征、结构相似性等轻量特征
- 图片文本内容相似度
  - 比较网站图识别文本与参考图识别文本
- 图片布局/主体接近度
  - 允许因 Shopify 压缩、轻裁边、导出格式不同而产生的小幅差异

### 第一版落地建议

为控制复杂度，第一版建议使用“两段式判断”：

1. 先做轻量视觉近似筛选，找出每张网站图最相近的前 1-3 张参考图
2. 再用 Gemini 对“网站图 + 候选参考图”做判定，输出：
   - 是否可视为同一张素材的不同导出版本
   - 差异是否在可接受范围内
   - 如果不是同一素材，说明主要差异点

这样可以避免单纯依赖像素或 OCR，提升在压缩、轻裁边场景下的稳健性。
...
```json
{
  ...
  "reference_reason": "主体构图和文案内容高度一致，差异主要来自站点压缩和尺寸变化"
}
```
