...
- 结果数据和卡片展示优先使用 AI 中文对照字段：
  - 美国分析优先展示 `summary_zh`，缺失时回退 `summary`。
  - 欧洲分析优先展示 `reasoning_zh`、`strengths_zh`、`risks_zh`、`required_changes_zh`，缺失时回退英文原字段。
  - 完整返回报文仍保留原始 JSON，方便核对中英文对照和模型原始输出。
