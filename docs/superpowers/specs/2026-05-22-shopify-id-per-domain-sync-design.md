...
6. 产品 API、按 Shopify ID 反查产品、SKU 配对同步等读取链路必须同时读取
   `media_product_shopify_ids`，避免非默认域名 ID 已存档但业务仍按单字段漏匹配。
...
4. 旧字段兼容：
   ...
   - 已有且不一致时不覆盖，写入报告冲突。
5. 读取闭环：
   - `/medias` 产品序列化继续返回旧 `shopifyid` 字段，同时新增
     `shopify_ids` 列表和 `shopify_ids_by_domain` 映射。
   - `find_product_ids_by_shopifyid()` 同时查询旧字段与
     `media_product_shopify_ids.shopify_product_id`。
   - 店小秘 SKU 配对同步和单产品手动刷新 SKU 的本地 Shopify ID 集合必须合并旧字段与
     per-domain 存档表，同一产品多个 Shopify ID 聚合后一次写入 `media_product_skus`。
   - 明空产品库等候选匹配逻辑读取产品 Shopify ID 时，也要把 per-domain 存档表加入候选。
