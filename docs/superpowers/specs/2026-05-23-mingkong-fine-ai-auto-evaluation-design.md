...
5. 优先处理明空 `视频素材库` 按 90 天消耗倒序 Top500 视频卡片。
6. Top500 没有可跑任务后，再处理 `昨天消耗前300` 归档中的前 100 张视频卡片。
...
生产观察显示单张卡片真实执行时长通常在 4-6 分钟。改为常驻任务池后，吞吐由“每 10 分钟最多 2 张”变为“多个卡片同时跑，跑完即补位”。2026-05-24 生产观察未出现 `429` 后，任务池先从 2 张提高到 4 张；首个 30 分钟窗口仍未出现 `429` 后，再提高到 6 张。随后 6 并发在 ADC Gemini 3.5 Flash 通道出现明确 `429 RESOURCE_EXHAUSTED`，回退 4 后短窗口仍有 429，一度降到 2。运营确认后，默认并发固定为 4；429 只记录观察，不触发自动降级。单张卡片内部的国家评估默认仍保持串行，避免 ADC 通道突然增加国家级并发。

2026-05-25 将候选范围回调到视频素材库 Top500 和昨天消耗前 100 后，worker 并发仍保持 4。`scheduled_tasks` 登记、代码默认值和 systemd unit 的 `--workers` 参数必须一致，避免发布后实际并发漂移。
...
- `docs/superpowers/specs/2026-05-18-mingkong-daily-material-snapshot-top100-design.md`：本地明空视频素材库和昨天消耗 Top300 的事实表；自动评估只读取其中前 100 张作为候选。
...
当 Top500 中没有可跑任务后，再从最新归档的 `mingkong_material_daily_top100` 取前 100 张：
...
这里取 `昨天消耗前300` 展示归档中的前 100 张，不只取 `is_new_top100_entry=1` 的新进卡片。表名和字段名保留 `top100` 是兼容历史迁移。
...
- 在 Top500 内重复，只取一次。
- 同时出现在 Top500 和昨天消耗前 100，只按 Top500 优先级跑一次。
...
- `id`
- `material_key`
- `source_bucket`：`top500_90d_spend` 或 `yesterday_top100`
- `source_rank`
- `product_code`
- `product_url`
- `mk_product_id`
...
3. 如果 Top500 没有可领取候选，查询昨天消耗前 100 候选。
...
- service 测试确认 Top500 优先、Top500 耗尽后取昨天消耗前 100。
