...
生产观察显示单张卡片真实执行时长通常在 4-6 分钟。改为常驻任务池后，吞吐由“每 10 分钟最多 2 张”变为“多个卡片同时跑，跑完即补位”。2026-05-24 生产观察未出现 `429` 后，任务池先从 2 张提高到 4 张；首个 30 分钟窗口仍未出现 `429` 后，再提高到 6 张。随后 6 并发在 ADC Gemini 3.5 Flash 通道出现明确 `429 RESOURCE_EXHAUSTED`，默认并发回退到 4。单张卡片内部的国家评估默认仍保持串行，避免 ADC 通道突然增加国家级并发。
