...
2. 每个视频素材按素材名匹配所有广告 AD，并聚合该素材的：
   ...
   - 最近 30 天消耗
   - 总 ROAS
   - 今天 / 昨天 / 最近 7 天 / 最近 30 天 / 总成效数（列表展示为“订单量”）
...
- `总消耗` 单元格内部渲染为四行六列的小表格：第一列是说明列，第一行依次是 `说明`、`今天`、`昨天`、`7天`、`30天`、`总消耗`；第二行是 `广告消耗` 和对应金额；第三行是 `ROAS` 和对应时间窗口 ROAS；第四行是 `订单量`，展示该素材匹配广告 AD 的 `result_count` 成效数；不显示额外的 `总计` 标签。
