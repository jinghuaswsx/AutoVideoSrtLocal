...
回填工具默认只处理 `product_assets_synced_at` 为空、`product_code` 为空、`product_main_image_url` 为空或 `product_main_image_object_key` 为空的商品，避免重复抓取已完整补齐的商品。`--dry-run` 只统计候选商品和影响行数，不访问商品页、不访问明空、不写数据库；`--limit` 用于小批试跑；`--force` 只在需要重跑已尝试商品时手动使用。
