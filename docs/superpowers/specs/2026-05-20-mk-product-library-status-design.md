...
1. 在明空产品库表格新增「产品状态」列，第一行显示 `已入库` 或 `未入库`。
...
- 渲染行时读取 `r.library_status`：
  - 第一行 badge 显示 `已入库 / 未入库`。
  - 第二行固定展示判色依据：`近一个月消耗`、`ROAS`、`保本ROAS`；缺失值显示 `--`。
...
7. 明空产品库表格新增「产品状态」列，空数据 colspan 正确。
8. 「产品状态」列第一行显示入库状态，第二行显示近一个月消耗、ROAS 和保本 ROAS。
