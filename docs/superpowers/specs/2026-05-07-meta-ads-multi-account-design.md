...
4. 配置存在数据库 `system_settings.meta_ad_accounts`（JSON），可在 admin 设置页之后再加 UI；本期允许直接 SQL 修改。
5. newjoyloo 配置保留但 `enabled=false`，Omurio 配置 `enabled=true`，解封后只需把 enabled 翻成 true。

## 非目标

- 不做 admin UI 编辑界面（后续单独 ticket）。
...
历史 CSV 文件**不动**（旧 newjoyloo CSV 在原位置保留），新跑的 CSV 进 subfolder。如果 import 逻辑只读 `_run_meta_ads_manager_export` 返回值的路径，无需迁移。
...
1. 应用 SQL `db/migrations/2026_05_07_meta_ad_accounts_setting.sql`：
   ...
   - 包含 newjoyloo (enabled=false) + Omurio (enabled=true) 两条种子。
2. 部署代码（按本机部署 SOP）。
