...
- 文案封面详情页在窄屏移动端必须默认隐藏顶部进度区内的 4 个步骤状态卡片，避免 sticky 顶部区域遮挡结果内容。
- 顶部进度区必须提供“展开步骤 / 收起步骤”切换按钮；按钮只在移动端可见，使用 `aria-expanded` 反映当前折叠状态，并控制 `vcdProgressSteps` 的显示。
- 桌面端仍保持 4 个步骤状态卡片默认展开；移动端展开或收起只影响前端显示，不改变后端步骤状态、轮询、重试、提示词或报文预览逻辑。
- 轮询刷新时必须继续更新被隐藏的步骤状态卡片，用户展开后看到的是当前最新状态和消息。
- 移动端顶部进度区不得继续使用 `position: sticky` 悬浮。移动浏览器滚动时地址栏和可视高度会变化，sticky 卡片会出现抖动；窄屏应使用普通文档流展示，桌面端仍保留 sticky。
...
- 文本步骤的模型池必须按场景给出多个可选模型。视频分析可选 Gemini 3.5 Flash、Gemini 3 Flash、Gemini 3.1 Flash-Lite；产品分析和文案创作同样提供 Gemini 3 系列，其中文案创作在 OpenRouter 下额外提供 Claude Sonnet、GPT-5.5、GPT-5 Mini 等文本模型。
...
- 视频分析：默认 `GOOGLE VERTEX ADC` / `gemini-3.5-flash`；OpenRouter 对应 `google/gemini-3.5-flash`。同供应商还可选择 Gemini 3 Flash 和 Gemini 3.1 Flash-Lite。
- 产品分析：默认 `OPENROUTER` / `google/gemini-3-flash-preview`；Google Vertex ADC 对应 `gemini-3-flash-preview`。同供应商还可选择 Gemini 3.5 Flash 和 Gemini 3.1 Flash-Lite。
...
- 封面生成：默认 `本地接口` / `gpt-image-2`，本地接口默认 base URL 为 `http://172.16.254.106:82/v1`，API key 存放在 `llm_provider_configs.video_cover_local_image`；OpenRouter 可选 OpenAI Image 2 low / mid / high、Nano Banana 2、Nano Banana Pro、Nano Banana 1 兜底。本地接口可选 GPT-Image-2、Nano Banana 2、Nano Banana Pro、Nano Banana 1 兜底。
...
### 2026-05-22 商品抓取 403 处理修订

- 文案封面创建项目时，商品页面和 Shopify `.json` 抓取必须使用完整浏览器请求头，避免部分 Shopify / Cloudflare 站点拒绝过短的 `Mozilla/5.0` User-Agent。
- 商品抓取阶段的 HTTP / 网络异常必须包装成 `VideoCoverGenerationError`，由 `POST /video-cover/api/projects` 返回 400 JSON，不允许冒泡成 Flask 500 或让前端只能显示泛化“创建失败”。
