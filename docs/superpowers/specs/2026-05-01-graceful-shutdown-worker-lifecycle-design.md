...
- 长任务统一登记到 [`appcore/runner_lifecycle.py`](../../../appcore/runner_lifecycle.py) 的 `start_tracked_thread(daemon=False)`；活跃任务记在 [`appcore/task_recovery.py`](../../../appcore/task_recovery.py) 的内存 set `_active_tasks`，进程退出后自然丢失。
