...
移动端访问「数据分析 → 实时大盘 → 广告计划」时，表头和数据列错位：表头按「广告计划 / 账户 / 消耗 / 结果数 / 平台购买价值」等短 label 计算列宽，tbody 则按长广告计划名和账户名计算列宽，导致数据看起来落在错误列下。
...
- 实时大盘所有 `.oa-table-scroll .oa-table` 在移动端仍保持一个完整 table layout，由外层 `.oa-table-scroll` 负责横向滚动。
...
- `#panelRealtime .oa-table-scroll table.oa-table:not(.mobile-no-scroll)` 恢复 `display: table`，`width: max-content`，`min-width: 100%`，`max-width: none`。
...
- 静态回归：`tests/test_order_analytics_template_layout.py` 检查页面级 CSS 覆盖和 `oar-campaign-table` class 存在。
