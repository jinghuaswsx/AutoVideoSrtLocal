...
4. 采购配对确认阶段如果只有部分 SKU 阻断，已确认或已配对的 SKU 仍要继续执行小秘云仓添加商品和采购价刷新；最终响应保留阻断状态，不能让单个缺失采购配对阻断其它可用 SKU 入库。
5. 物流与包装同步失败必须在逐 SKU 结果中可见，不能静默吞掉；同步成功后继续走 1688 配对、小秘云仓添加商品、采购价刷新等后续闭环。
