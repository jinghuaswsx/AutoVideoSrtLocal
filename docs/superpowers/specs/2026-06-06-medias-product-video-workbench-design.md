...
2026-06-11 修订：本地素材库中 `lang = 'en'`，且 `filename` 或 `display_name` 去扩展名后以 `-蔡靖华` 结尾的素材，是自营美国站跑出的可迁移 EN 原素材。即使该产品没有同步到明空视频素材，也必须作为已入库来源卡片进入工作台：

1. 这类卡片来源标记为 `local_en_cjh`，直接使用本地素材 `media_items.object_key` 播放视频，不走明空视频代理。
2. 卡片状态按“已入库/自营素材”处理，不展示“加入素材库”动作，可直接创建小语种任务。
3. 这类素材可作为欧洲/日本等多国家翻译任务的源头，翻译版本仍按 `source_raw_id` / `source_ref_id` / 历史命名指纹归并。
4. 统计上不计入“明空素材”总数；但计入可操作卡片和已入库素材数量。
5. 如果同一个本地素材已经被明空卡片通过绑定表或历史精确匹配展示，不再额外生成自营来源卡片，避免重复操作入口。
...
2026-06-11 修订：历史高置信度匹配中的文件名精确比较允许忽略分隔符周围的无意义空格，例如明空 `2025.09.16- 马鞍式牵引遛狗绳-原素材-苏齐齐.mp4` 与本地 `2025.09.16-马鞍式牵引遛狗绳-原素材-苏齐齐.mp4` 视为同一文件名。该规则只折叠 `-` / `_` 等分隔符两侧空白，不做关键词、日期、人员或产品名模糊推断。
