...
随着业务对"单品盈亏全景"的诉求加深，需要在同一页里同时回答 5 个问题：哪些产品在赚 / 亏（横向比较）、单品的订单逐笔盈亏、国家销售入口、单品的广告投放效率、单品在各国家之间的订单量 / 销售额 / 利润 / ROAS 横向对比。当前堆叠式结构无法承载这些视角，本设计把它重构为 **5 Tab 单页**。

## 2. 目标

- **零数据丢失**：现有看板的所有数据视图（总账卡片 / 站点切片 / 每日折线 / 国家柱状图 / 15 列订单明细 / Excel 下载）必须在新版本里全部找得到，不削减。
- **统一视角**：4 个 Tab 共用同一筛选上下文（产品 / 日期 / 国家），一处选择，多处生效。
- **入口友好**：默认 Tab 展示全产品盈亏概览，产品搜索框不预设具体产品，无需先选产品也能用。
...
筛选条不再作为全局固定语义，而是跟随当前 Tab 显示不同控件。底层仍复用同一组选中状态，避免切换 Tab 时丢失产品 / 日期 / 国家。

| Tab | 筛选控件 | 说明 |
|---|---|---|
| ① 产品列表 | 日期范围、国家、查询 | 产品列表始终展示全产品聚合，不显示产品选择 |
| ② 订单明细 | 产品、日期范围、国家、查询、下载 Excel | 必须选具体产品 |
| ③ 国家看板 | 产品、日期范围、查询 + Tab 内容内国家胶囊 | 产品可选；国家用胶囊按钮而非顶部下拉 |
| ④ 广告明细 | 产品、日期范围、国家、查询 | 必须选具体产品 |
| ⑤ 产品国家分析 | 产品、日期范围、查询 | 必须选具体产品；不受顶部国家筛选影响 |

- Tab 切换器改为胶囊按钮状态，放在页面标题“产品盈亏看板”正下方，筛选条在胶囊按钮下方。
- 产品控件使用只读输入框 + modal 选择器，不再依赖浏览器 `datalist`：顶部输入框展示当前产品，宽度比旧版加 50%（桌面端从 320px 到 480px），点击后直接弹出产品选择 modal。
- 产品选择 modal 顶部是搜索栏，打开后光标自动落入搜索栏；搜索为空时展示所有真实产品，产品列表按 `product_code` A-Z 排序；搜索时匹配 `product_code` 和产品名。
- modal 内保留「全部产品」快捷项，但真实产品列表仍按 `product_code` 排序展示；点击具体产品后回填顶部输入框、关闭 modal。若当前是「产品国家分析」Tab，继续停留并刷新该 Tab；其他 Tab 默认跳转到「订单明细」Tab 查询该产品。点击「全部产品」后回填顶部输入框、关闭 modal，并回到「产品列表」Tab。
- 进入 `/product-profit` 时产品搜索框默认保持空输入，不自动填入任一具体产品；只有 URL 带合法 `product_id` 或用户点击产品行 / 手动选择后，才写入具体产品。
...
国家看板不使用柱状图。顶部固定展示国家胶囊按钮，点击国家后直接展示该国家的销售情况。

胶囊来源：
- 固定包含美国 `US` 和英国 `GB`，其中 `US` 放第一个胶囊，`GB` 放第二个胶囊。
- 其余来自当前启用的小语种国家，沿用 [docs/superpowers/specs/2026-04-28-order-analytics-monthly-redesign-design.md](2026-04-28-order-analytics-monthly-redesign-design.md) 的“启用国家列表的推导”口径：`media_languages.enabled=1` → `COUNTRY_TO_LANG` 反推国家。
- 本页最多展示 9 个胶囊；美国和英国各占一位，其余按启用语种排序取主要国家。后续如果需要超过 9 个或手工配置顺序，再引入配置化国家列表。

选中某国后的行为：

```
┌─ 国家胶囊：[美国 US] [英国 GB] [德国 DE] [法国 FR] ...
├─ 未选择产品:
│   该国产品销售排行表，按订单数从高到低排序
│   列：产品 / 订单数 / 收入 / 物流 / 采购 / 广告 / ROAS / 利润 / 成本完备
└─ 已选择具体产品:
    聚合卡片（该国该产品订单数 / 件数 / 收入 / 利润 / ROAS）
    该国该产品订单明细（前 100 行）
```

数据来源：
- 未选择产品：复用 `GET /order-analytics/product-profit/list.json?country=<country>`，前端按 `order_count DESC` 排序。
- 已选择产品：复用 `GET /order-analytics/product-profit/report.json`，前端按 `buyer_country` 过滤订单后重算聚合。
- 胶囊列表：新增 `GET /order-analytics/product-profit/countries.json`，只返回本页需要展示的国家胶囊。
...
├─ Campaign 列表（按账户分组）
│   列：[账户] [Campaign] [花费] [展示] [点击] [结果] [CTR] [CPC] [归属订单] [归属收入] [ROAS] [利润贡献] [操作]
│   排序默认按花费降序
│   仅 campaign_product_overrides 中存在人工绑定的 campaign 显示 [解绑]；自动匹配项不显示解绑动作
├─ 日趋势子图（折叠区，默认收起）
│   每日花费 vs 当日归属收入 双线
└─ 未匹配 campaign 折叠区
...
- `campaign_product_overrides.id` 作为人工绑定的解绑依据；解绑只删除人工 override 并清空对应广告事实表归属，自动匹配不提供解绑按钮
- 归属订单 / 归属收入 = 同日同产品的订单聚合（口径与现有 `allocate_ad_cost_to_line()` 一致）
- 利润贡献 = 归属收入 - 该 campaign 花费 - 同日同产品的采购 + 物流分摊

## 10. Tab ⑤ 产品国家分析（新增）

只有选择具体产品时展示数据；未选产品或选择“全部产品”时显示空状态：“请先选择具体产品后查看产品国家分析。”

展示为国家横向对比矩阵，列是国家，行是关键指标：
...
数据来源：
- 复用 `GET /order-analytics/product-profit/report.json`。
- 前端按 `by_country` 的 `buyer_country` 聚合跨站点数据。
- 国家列按销售额从高到低排序。
- ROAS = 销售额 / 广告费；广告费为 0 时显示 `--`。
- 本 Tab 不读取顶部国家筛选，目的是让同一个产品在所有国家之间横向比较。
...
| `/product-profit` | **产品维度盈亏全景看板（本设计的 5 Tab）** | 产品负责人 / 经理 |
...
- Tab 切换器：胶囊按钮风格，白底描边，激活态 `--accent` 底 + 白字，紧挨页面标题下方
...
- [ ] 顶部产品输入框桌面端宽度为旧版 1.5 倍，点击后弹出产品选择 modal，modal 搜索栏自动获得焦点
- [ ] 产品选择 modal 搜索为空时显示所有产品，并按 `product_code` A-Z 排序
- [ ] `/product-profit` 首次进入时产品搜索框不自动填入任一具体产品
- [ ] Tab 切换器是胶囊按钮，放在页面标题下方、筛选条上方
- [ ] 不同 Tab 显示不同筛选控件：产品列表不显示产品选择；国家看板不显示国家下拉；产品国家分析只显示产品 + 日期 + 查询
- [ ] 顶部产品下拉切换为具体产品时，②③④⑤ Tab 可查看对应数据
- [ ] 顶部产品下拉为"全部"时，②④⑤ Tab 显示"请先选择产品"空状态，③ Tab 按选中国家展示产品销售排行
- [ ] 「产品列表」点击产品行 → 自动锁定该产品 + 切到「订单明细」
- [ ] 「订单明细」Tab 包含旧版的所有 5 个区块（聚合卡片 / 每日折线 / 站点切片 / 15 列订单表 / Excel 下载），数值与旧版逐项对账一致
- [ ] 「国家看板」Tab 不显示柱状图，显示 9 个国家胶囊按钮（美国第一、英国第二，后续为启用小语种国家）
- [ ] 「国家看板」未选产品时，点击国家后按该国家订单数从高到低展示产品销售排行
- [ ] 「国家看板」选了产品时，点击国家后展示该国家该产品订单数、件数、收入、利润和订单明细
- [ ] 「广告明细」Tab 列出 campaign 级聚合、按账户分组、有日趋势 + 未匹配区
- [ ] 「产品国家分析」Tab 只有选择产品后才展示矩阵，矩阵列为国家，行为订单量 / 销售额 / 利润 / ROAS
- [ ] 未匹配区的"手工配对"按钮调用现有 `manual_match_meta_ad_campaign()`
- [ ] Campaign 明细最右侧仅对人工绑定项显示"解绑"按钮；解绑后该 campaign 回到未匹配 / 自动匹配状态
...
- [ ] 部署到测试环境（172.16.254.106:8080）端到端验证
- [ ] pytest 新增 / 修改的测试全部通过
