...
- 在 `openrouter` 通道下按配置动态暴露 1 个图片翻译模型备选项
- 在运行时把该备选项映射到 OpenRouter 模型，并固定透传 `quality=low`
...
采用“单一 Low 档虚拟模型 + 后台开关”的方案：

- 用户侧仍然只看见图片翻译模型备选项
- `low` 表示为唯一可选择的 `model_id`
- 运行时把该 `model_id` 解析为 OpenRouter 底层模型和 `quality=low`
...
- OpenAI 官方图像文档当前底层图像模型为 `gpt-image-2`，支持多档 `quality`；图片翻译模块固定只用 `low`
...
- 图片翻译 UI 只展示 Low，运行时也只传 `quality=low`
...
- 开关关闭时，“默认质量”控件禁用或隐藏
- 开关开启时，“默认质量”固定为 `low`；图片翻译不暴露 `mid / high`

### 4.2 图片翻译新建页

在 [web/templates/_image_translate_scripts.html](G:\Code\AutoVideoSrtLocal\.worktrees\codex-openrouter-openai-image2\web\templates\_image_translate_scripts.html) 中，模型 pill 组继续使用现有渲染逻辑，但在 `openrouter` 通道且开关开启时只追加 1 个可选模型：
...
[web/routes/medias.py](G:\Code\AutoVideoSrtLocal\.worktrees\codex-openrouter-openai-image2\web\routes\medias.py) 的默认模型逻辑同步支持 Low 模型项，但是否可以作为新任务默认值仍受后台开关控制。
...
- 详情页继续显示原模型 ID
- 重新生成和重试在图片翻译模块内强制按 Low + 1K 执行，避免旧 `mid / high` 继续产生高费用
- 即使管理员后来关闭了该功能，也不影响旧任务按 Low 安全执行
...
- `default_quality`：字符串，仅允许 `low`，默认 `low`；保留字段只是为了兼容旧配置表单

2026-05-19 默认策略更新：

- 图片翻译全局默认通道改为 `openrouter`。
- 商品图片翻译默认模型改为 `openai/gpt-5.4-image-2:low`，并默认并行执行。
- 图片翻译模块的 OpenRouter OpenAI Image 2 请求必须固定透传 `quality=low`、`extra_body.image_config.image_size=1K`，并按源图推导 `extra_body.image_config.aspect_ratio`，避免默认 1:1 和 2K 高费用请求。
- 生产环境已有 `system_settings` 需要通过迁移覆盖默认通道、默认模型、开关和默认质量，避免历史保存值继续指向旧通道。
- API 账单价目表需要补 `ai_model_prices`：基础模型 `openai/gpt-5.4-image-2` 记录 OpenRouter token 价（input $8/M、output $15/M，汇率 6.8），图片翻译仅使用虚拟模型 `:low` 的图片 fallback 价。OpenRouter 响应 `usage.cost` 优先，价目表只作为后台展示和 response cost 缺失时的兜底。
...
### 5.4 2026-05-19 可选档位收敛

新建图片翻译任务和设置页只继续暴露 `OpenAI Image 2（Low）`。`Mid` / `High` 不再出现在模型下拉、模型接口或默认质量控件中，也不能作为新任务的可选模型提交。

历史任务兼容性保持不变：已经保存的 `openai/gpt-5.4-image-2:mid` / `openai/gpt-5.4-image-2:high` 保留原 `model_id` 用于详情展示；图片翻译执行路径会强制转为 Low + 1K，避免旧任务继续产生 Mid / High 高费用请求。
...
- 若配置开启，则在 `openrouter` 通道列表后追加 1 个 OpenAI Image 2 Low 模型
...
- 开关开启时，仅 `openai/gpt-5.4-image-2:low` 视为当前通道合法的新任务模型
- `openai/gpt-5.4-image-2:mid` / `openai/gpt-5.4-image-2:high` 不再通过新任务接口校验
...
- 继续按“当前图片翻译通道”返回默认模型
- 如果当前通道是 `openrouter` 且功能开启，则默认值只允许落到 `openai/gpt-5.4-image-2:low`
...
- 对图片翻译的 OpenAI Image 2 模型，在请求前强制解析为真实模型、`quality=low`、`image_size=1K` 和源图比例
...
client.chat.completions.create(
    ...
    extra_body={
        "quality": "low",
        "image_config": {"image_size": "1K", "aspect_ratio": "9:16"},
        "usage": {"include": True},
    },
)
```
...
- 开关开启：模型列表只追加 Low 档位，默认选中项为 `openai/gpt-5.4-image-2:low`

### 7.2 历史任务

- 历史任务中的 `model_id` 原样保留
- 详情页继续展示原 `model_id`
- 图片翻译执行、重试时若遇到历史 `mid / high`，运行时强制按 Low + 1K 执行
- 功能开关只影响“新建任务入口”，不阻断历史任务运行
...
- 默认质量配置为历史 `mid` / `high` 时，设置层回退到 `low`
...
- [tests/test_gemini_image.py](G:\Code\AutoVideoSrtLocal\.worktrees\codex-openrouter-openai-image2\tests\test_gemini_image.py)
  ...
  - OpenRouter 调用时固定传 `quality=low`、`image_size=1K` 和源图 `aspect_ratio`
  - 普通 Gemini OpenRouter 模型不受影响
  - OpenRouter 模型列表开启时只暴露 `OpenAI Image 2（Low）`
...
- [tests/test_image_translate_routes.py](G:\Code\AutoVideoSrtLocal\.worktrees\codex-openrouter-openai-image2\tests\test_image_translate_routes.py)
  ...
  - 新建任务时只允许 `openai/gpt-5.4-image-2:low`，拒绝 `mid / high`
...
按项目约定，在测试环境 `http://172.16.254.106:8080/` 验证：

1. 打开开关并设置默认质量
2. 在图片翻译页确认只出现 Low 档模型
3. 创建 Low 档任务，确认请求固定 `quality=low`、`image_size=1K`
4. 确认任务详情页能看到正确的模型标识
