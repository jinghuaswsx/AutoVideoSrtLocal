...
- 新建任务只暴露 `low` 档；运行时保留历史 `mid / high` 解析能力
- 作为一个可选配置项由后台控制启用与否
- 后台默认质量档位收敛为 `low`
...
- 在 `openrouter` 通道下按配置动态暴露 `OpenAI Image 2（Low）`
- 在运行时把历史 3 个虚拟 ID 映射到同一个 OpenRouter 模型，并透传不同质量参数
...
采用“Low-only 新建入口 + 历史三档虚拟 ID 解析 + 后台开关”的方案：

- 用户侧仍然只看见图片翻译模型备选项
- 新建入口只把 `low` 表示为可选择的 `model_id`
- 运行时继续把历史 `low / mid / high` 3 个 `model_id` 解析为同一个 OpenRouter 底层模型和不同 `quality`
...
- 运行时继续识别历史 `low / mid / high`，其中 `mid` 映射为 OpenAI API 的 `medium`；新建任务 UI 只暴露 `low`
...
[web/routes/medias.py](G:\Code\AutoVideoSrtLocal\.worktrees\codex-openrouter-openai-image2\web\routes\medias.py) 的默认模型逻辑只把 `openai/gpt-5.4-image-2:low` 作为新任务默认值。
...
- `enabled`：布尔值，默认 `true`
- `default_quality`：字符串，允许 `low`，默认 `low`；历史保存的 `mid / high` 读取时回退为 `low`
...
新建任务只使用 `openai/gpt-5.4-image-2:low`；`mid` / `high` 只用于历史任务运行时解析和既有计费展示，不再用于：
...
历史任务兼容性保持不变：已经保存的 `openai/gpt-5.4-image-2:mid` / `openai/gpt-5.4-image-2:high` 在运行时仍按原档位解析，避免旧任务详情、重试或后台执行被破坏。
...
- 统一做默认值回退
- 统一做 `low` 枚举校验，并把历史 `mid / high` 默认质量回退到 `low`
- 为设置页和模型注册层提供稳定接口
...
- 若配置开启，则在 `openrouter` 通道列表后只追加 `OpenAI Image 2（Low）`
- `Mid` / `High` 只作为历史任务运行时解析值保留，不进入新建任务候选列表
...
1. 打开开关并设置默认质量
2. 在图片翻译页确认仅出现 `OpenAI Image 2（Low）`
3. 创建 `low` 任务
4. 确认任务详情页能看到正确的模型标识
