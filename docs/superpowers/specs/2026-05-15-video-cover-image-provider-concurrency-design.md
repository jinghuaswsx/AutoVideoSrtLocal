...
- APIMART 和本地 Image 2、Google Vertex ADC 一样固定串行；只有 OpenRouter 支持并发开关。
...
- 不为本地 Image 2 或 Google Vertex ADC 提供并发开关。
...
- `provider=local_image_2` 或 `provider=gemini_vertex_adc`：忽略提交值并归一为 `serial`。
...
- 供应商下拉：本地 Image 2、OPENROUTER、GOOGLE VERTEX ADC。
...
- 非 OpenRouter 即使收到 `parallel` 也强制串行，避免本地 Image 2 或 Vertex ADC 被意外并发打爆。
...
- 第四步选择本地 Image 2 或 Google Vertex ADC 时不显示执行方式下拉，保存后状态为 `serial`。
