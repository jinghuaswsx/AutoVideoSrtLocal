...
部署或重装 CDP 环境时，必须同步清理历史遗留的 systemd drop-in（例如 `/etc/systemd/system/autovideosrt-roi-realtime-sync.service.d/10-browser-lock.conf`）。否则即使主 unit 已经改为直连 DXM01/DXM03，drop-in 仍会覆盖 `ExecStart`，导致「店小秘订单与 ROAS 实时同步」继续等待旧共享锁并超时。
