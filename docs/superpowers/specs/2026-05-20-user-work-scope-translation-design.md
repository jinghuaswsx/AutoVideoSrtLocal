...
给用户账号增加可扩展的“工作范围”标记，第一期只落地 `翻译工作`。明空选品页的“加入素材库”和“做小语种”两个指派弹窗只展示具备 `翻译工作` 范围的用户；`superadmin` 不再因角色被过滤，仍按“全部权限开启”可接翻译任务。初始化 5 个指定员工拥有该范围。
...
3. 新增应用层 helper，返回可接收翻译工作的用户列表；普通用户列表必须同时满足：
   - `users.is_active = 1`
   - `permissions.can_translate = true`
   - `permissions.work_scope_translation = true`
   - `superadmin` 只要 `is_active = 1` 就可出现在列表中，因为权限体系约定超管永远视为全部权限开启。
...
角色默认值：
- `superadmin`：仍由现有逻辑全部为 true，并允许被指派翻译任务。
...
2. 打开 `选品中心 → 明空选品`。
3. 点击任意卡片 `加入素材库`，下拉只展示：周干琴、顾倩、王舒溦、王健、蔡靖华；其中蔡靖华可以是 `superadmin`。
4. 点击 `做小语种`，翻译员下拉同样只展示这 5 人，且可选择 `superadmin` 接任务。
