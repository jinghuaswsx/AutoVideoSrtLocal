...
- `push-module/` 是从另一个项目拆出来的包（见 `push-module/README.md`）。包含两个 React + JSX 组件：
  - `PushCreate.jsx`：按 `product_code` 从外部 AutoVideo OpenAPI（`http://172.16.254.106`）拉素材，展示并让用户编辑推送 JSON；当前版本没有"推送"按钮。
...
- **风险 A — CORS**：验证路径通不通的核心拦路虎。按 push-module README 第 2.1 节，浏览器直连 `172.16.254.106` 和 `172.17.254.77:22400` 需要两个服务端都加 `Access-Control-Allow-*` 头。本 MVP 不解决 CORS，跑不通时由用户决定是否让对方服务端加头，或回退走后端代理。
