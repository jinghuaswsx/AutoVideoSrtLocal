...
## 2026-05-08 修复口径：订单快照优先级

实时大盘单日视图使用 `roi_realtime_daily_snapshots` 时，必须优先选择 `snapshot_at` 最新的快照作为订单事实来源。`ad_data_status` 只能影响广告费、Meta ROAS、真实 ROAS 和广告新鲜度提示，不能让页面回退到更早的订单快照。

已知事故：2026-05-08 订单同步已经把 5.8 订单写入 `dianxiaomi_order_lines`，最新快照 `2026-05-08 11:00` 有 57 单，但因为广告同步失败导致 `ad_data_status='pending_source'`，接口优先选择了 `2026-05-07 22:40` 的旧 `ad_data_status='ok'` 快照，前台只展示 9 单。修复后应展示最新订单数，同时保留广告状态为待更新。
...
2026-05-29 补丁：页面新增全局 / 新品 / 老品 / 未匹配四组 scope 汇总卡后，时间水位卡片（订单最新时间、订单数据更新时间、广告数据更新时间、数据快照时间）必须放在 `#realtimeSummary` 最顶部，位于四组 scope 汇总卡之前。时间字段口径不变：`订单最新时间` 仍表示所选业务日期范围内最后一单，`订单数据更新时间` 表示订单同步 / 订单行最新落库时间，`广告数据更新时间` 表示广告同步最新完成或广告行最新更新时间，`数据快照时间` 表示当前数据截止快照点。
...
4. **手动**：登录 `http://172.16.254.106`（使用 `testuser.md` 中的管理员凭据），切到「数据分析」 tab，确认 PC + Chrome DevTools mobile 都正常。
