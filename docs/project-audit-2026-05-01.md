...
- 2026-05-06 当前工作区继续收敛 P2-14 大文件拆分：素材 item 封面缓存与上传完成后的缩略图生成、时长读取、`media_items.thumbnail_path/duration_seconds` 写入已从 `web/routes/medias/items.py` 下沉到 `web/services/media_items.py`，路由仅注入缓存根目录和对象下载函数；同步补齐 service 行为测试和架构边界测试，防止 DB/ffutil 副作用回流到 route 模块。聚焦回归：`tests/test_media_items_service.py tests/test_medias_routes.py tests/test_architecture_boundaries.py` 为 `257 passed, 2 warnings`，`py_compile` 与 `git diff --check` 通过。
